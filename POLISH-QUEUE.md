...
- [ ] **§3.4 decision-tree boundary calibration — two soft-spots found during active-learning labeling (2026-05-28).** During the first live `auto_validate_from_resolutions` + `recommend_validation_target` session (5/5 positives confirmed; v1.0 progress 24% → 29%), two distinct §3.4 `classify()` boundaries surfaced where the tree falls through to under-informative verdicts despite strong joint-pair model evidence.

  **Soft-spot 1 — `scribCos < 0.5` cutoff in `same_composition_different_scribe` branch (`comparePair.ts:classify()` ~line 254).** `BM.38552 ↔ K.9270` had `scribCos = 0.503` (three thousandths above 0.5) with `fuzzy_J = 0.404` + `thematic = 0.87` + `longest_contiguous_run = 102` + 591 fuzzy-intersect trigrams. The `same_composition_different_scribe` branch requires `scribCos < 0.5` — just missed. The `same_scribe_different_composition` branch requires `scribCos ≥ 0.7` — well missed. Tree falls through to `weak_relationship [low]` despite overwhelming evidence. Joint-pair model correctly emits P=0.9419 positive. Suggests either relaxing strict-inequality to `≤ 0.5`, or widening the cutoff to `< 0.7` (i.e. `same_composition_different_scribe` is the right verdict whenever `fuzzyJ ≥ 0.3 && scribCos < 0.7`, with confidence dropping for scribCos in the 0.5–0.7 band).

  **Soft-spot 2 — `thematic_only` branch ignores `scribal_cos` entirely (`comparePair.ts:classify()` ~line 263).** `K.17494 ↔ K.47` had `thematic = 0.8432`, `scribal = 0.6970` (three thousandths below the 0.7 same-scribe threshold), `fuzzy = 0`, `substitution_lift_z = -8.76` (extreme low — strongest same-scriptorium sign-form signal of the session). Tree returns `thematic_only [medium]` because thematic_only conditions (`thematic ≥ 0.7 && fuzzy < 0.15 && exact < 0.05`) are met — but the 0.697 scribal signal is real near-same-scribe evidence the branch ignores. Joint-pair model correctly emits P=0.9621 positive (strongest of session). Suggests adding a guard: if `scribal_cos ≥ 0.6` while thematic_only conditions also match, prefer `same_scribe_different_composition [medium]` instead, since high thematic + high scribal + zero lexical fits "same scribe copying different texts in same compositional family" better than "thematic-only paraphrase."

  **Auditable evidence:** both labels persisted to `~/.cache/cuneiform-mcp/validation-resolutions.json` with `source: "validation_queue"` and rationale citing the specific boundary values. Active-learning trail in `docs/auto-validation-proposals-2026-05-27T21-50-14Z.md`. Both pairs landed positive after `explain_pair_score` review.

  **Bonus side-finding — active learning is implicit metadata enrichment.** Four of the five top-5 active-learning candidates (K.7603, K.9270, 1881,0204.198, K.17494) had partial or empty `genres_flat` in `getFragmentMetadata`. Each accepted positive is an implicit metadata hint (K.7603 ≈ Zi-pà; K.9270 ≈ Balaĝ; 1881,0204.198 ≈ Bārûtu/Ubānu; K.17494 ≈ Šumma Ālu). Future tool idea: surface these as proposed `genres_flat` enrichments — not a §3.4 fix, but a related observability win the same labeling process produces for free.
