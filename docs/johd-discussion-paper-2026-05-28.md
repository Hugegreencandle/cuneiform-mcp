...
The cluster reconstructor's output is structured enough that secondary frameworks — archetype typologies, transmission-lineage trees, scribal-school graphs — can be built on top of it. We do not develop those frameworks here; the point for present purposes is that the methodology should preserve this composability property under any future re-calibration. A re-calibration that improved aggregate sibling-rescue rate at the cost of producing less-structured cluster outputs would be a worse calibration overall.
