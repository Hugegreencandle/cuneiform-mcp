...
Confirming the 10 Tier-1 pairs would move G2 17 → ~27. The rest of the gap lives in Tier-2 (untagged `CANONICAL` candidates) and needs a **second signal** — chunk-overlap with the anchor — to become confirmable. (Honest caveat, per the v0.73 Genre-Sentinel review: chunk-overlap is **partially model-entangled**, because `identify_composition` is itself ~0.6-weighted on chunk-overlap — so it is a useful *generator* in a propose-only, human-confirmed loop, not a fully independent check.) That is the external-evidence rule below.

## Next: external-evidence rule (A2 step 2)

The genre-leaf-match used here is the genuinely independent signal (eBL editorial genre, external to every model); chunk-overlap is a second, **partially model-entangled** signal (`identify_composition` is chunk-weighted). An `auto_validate` rule should require **model-composition ≥0.95 AND (eBL genre leaf names the same composition OR chunk-overlap with a confirmed anchor ≥ threshold)**, with the threshold calibrated against the 17 known positives before it can write proposals. It rescues the untagged Tier-2 candidates (genre silent but textually overlapping a known sibling) — and stays honest only because it is propose-only and every hit is human-confirmed (the entangled signal never auto-records). Scoping pass to follow.
