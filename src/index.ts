...
import {
  identifyComposition,
} from "./identifyComposition.js";
import {
  buildStemmaWithRooting,
  type RootingMode,
} from "./recensionTreeRooted.js";
import {
  scoreTabletCompleteness,
} from "./scoreTabletCompleteness.js";
import {
  findCompositionLineage,
} from "./findCompositionLineage.js";
import {
  damagedPassageCompositionProbability,
} from "./damagedPassageComposition.js";
import {
  listCompositions,
  registryMetadata,
...
import {
  computeQuotationNetwork,
} from "./quotationNetwork.js";
import { surfaceGenreConflicts } from "./genreConflicts.js";
import {
  discoverCompositions,
} from "./discoverCompositions.js";
import {
  findLemmaParallel,
...
} from "./recalibrateLacunaScores.js";
import {
  recommendValidationTarget,
} from "./recommendValidationTarget.js";
import {
  listCandidateExemplars,
} from "./candidateExemplars.js";
import {
  getAncientFindSpot,
  getEblFragmentUrl,
  getEblPhotoUrl,
...
import { searchSumtablets } from "./sumtablets.js";
import {
...
import { diffCorpusVersions } from "./corpusDiff.js";
import { autoValidateFromResolutions } from "./autoValidateFromResolutions.js";
import { cdliEblCrosswalk } from "./cdliEblCrosswalk.js";
import { detectBilingualTablet, resolveGenrePrior } from "./bilingualDetect.js";
import { findBilingualTablets } from "./findBilingualTablets.js";
import {
  analyzeJoinsGraph,
...
function structuredResult<T>(
  ...
  // v0.62.0 — record into the session ring buffer for export_session.
  recordEnvelope({
...
server.registerTool(
  ...
  async ({ tablet_id, min_chunk_len, top_k, min_hosts, exclude_prefixes, cross_genre_only, cross_period_only }) => {
    ...
    try {
      ...
      for (const c of result.chunks) {
        // longest_contiguous_run << chunk_length + 2 ⇒ the span bridges X-damage
        // gaps (sparse alignment), NOT a single verbatim quotation. Flag it so
        // the run is never mis-read as a contiguous block.
        const spanSigns = c.chunk_length + 2;
        const sparse = c.longest_contiguous_run < spanSigns - 2;
        const contig = `longest contiguous run: ${c.longest_contiguous_run} signs${sparse ? " — SPARSE ALIGNMENT (spans gaps), not one verbatim block" : " (verbatim)"}`;
        lines.push(`── chunk ${c.chunk_key}  (span ${c.chunk_length} trigram positions ≈ ${spanSigns} signs; ${contig})`);
...
// ─── v0.61.0 — explain_pair_score (per-axis decomposition + calibration history) ─

server.registerTool(
  "explain_pair_score",
  {
    description:
      "Full provenance trace for any pairwise verdict. Returns the per-axis raw signals (lex_jaccard / fuzzy_jaccard / thematic_cosine / scribal_cosine / substitution_lift_z / composition_assignment_match) from compareTabletPair, the joint-pair model's per-feature additive decomposition (raw → z-standardized → × weight → contribution_to_log_odds), the cross-axis verdict from the methods-paper §3.4 decision tree, AND the full calibration history for each axis (every threshold change with version + date + source-doc path + rationale). Answers 'WHY did the model say X?' without forcing the caller to read v0.18.x-v0.60 release notes. Reuses compute_joint_pair_score + compare_tablet_pair — no new model or index. v0.61.0. Methods paper §3.x (T1-A upgrade plan).",
    inputSchema: {
      tablet_a: z.string().describe("Museum number of the first tablet."),
      tablet_b: z.string().describe("Museum number of the second tablet."),
    },
  ...
  async ({ tablet_a, tablet_b }) => {
    const SCHEMA = schemaId("explain_pair_score");
    try {
      const result = explainPairScore({ tabletA: tablet_a, tabletB: tablet_b });
      const lines: string[] = [
        `Pair: ${result.tablet_a}  ↔  ${result.tablet_b}`,
        `P(positive): ${result.pair_score.probability_positive.toFixed(4)}  ·  log_odds: ${result.pair_score.log_odds.toFixed(4)}  ·  classification: ${result.pair_score.classification}`,
        ``,
        `Decision summary:`,
      ];
      for (const s of result.decision_summary) lines.push(`  ${s}`);
      lines.push(``);
      lines.push(`Cross-axis verdict (methods paper §3.4 decision tree): ${result.verdict.primary_relationship} [${result.verdict.confidence}]`);
      for (const e of result.verdict.evidence) lines.push(`  · ${e}`);
      lines.push(``);
      lines.push(`Per-axis breakdown:`);
      for (const r of result.per_axis_explanation) {
        const latest = r.calibration_history.milestones[r.calibration_history.milestones.length - 1];
        lines.push(
          `  ${r.axis.padEnd(30)} status=${r.status.padEnd(16)} raw=${r.feature_value.toFixed(4)}  contrib=${r.contribution_to_log_odds.toFixed(4)}  [last calibrated: ${latest.version} → ${latest.source_doc}]`,
        );
      }
      lines.push(``);
      lines.push(
        `Model: ${result.model_metadata.version} — trained on n=${result.model_metadata.trained_on_n_positives} positives + n=${result.model_metadata.trained_on_n_negatives} negatives, accuracy ${result.model_metadata.training_accuracy.toFixed(4)}`,
      );
      if (result.model_metadata.warning) lines.push(`⚠ ${result.model_metadata.warning}`);
      if (result.warnings.length > 0) lines.push(`Warnings: ${result.warnings.join("; ")}`);
      return structuredResult(lines.join("\n"), {
        schema: SCHEMA,
        data: result,
        provenance: provenance("local", "local:explain-pair-score-calibration-trace", VERSION, {
          citation:
            "Per-axis decomposition + calibration-history provenance trace over compute_joint_pair_score + compare_tablet_pair. v0.59.0. Methods paper §3.x (T1-A upgrade plan).",
        }),
        warnings: result.warnings.length > 0 ? result.warnings : undefined,
      });
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      return structuredResult(`explain_pair_score error: ${msg}`, {
        schema: SCHEMA,
        data: {
          tablet_a,
          tablet_b,
          pair_score: { probability_positive: 0, log_odds: 0, intercept: 0, classification: "uncertain" },
          per_axis_explanation: [] as never[],
          verdict: { primary_relationship: "unrelated", confidence: "low", evidence: [msg] },
          model_metadata: {
            version: "unknown",
            build_timestamp: "",
          ...
          model_calibration_history: [] as never[],
          decision_summary: [msg],
          warnings: [msg],
        },
        provenance: provenance("local", "local:explain-pair-score-calibration-trace", VERSION),
        warnings: [msg],
...
// ─── v0.64.0 — auto_validate_from_resolutions (T1-B, PROPOSAL-ONLY) ───────
  ...
  async ({ mode, include_composition_siblings, composition_sibling_threshold, composition_sibling_min_conf }) => {
    ...
    try {
      ...
      const lines: string[] = [
        ...
        ``,
        `  Proposal file: ${result.proposal_file_path}`,
        `  Proposals:     ${result.proposals.length} (${result.proposed_positives} positive, ${result.proposed_negatives} negative)`,
        ``,
        `Rules applied:`,
      ];
      for (const r of result.rules_applied) {
        lines.push(`  - ${r.rule_id.padEnd(34)} ${r.proposals_generated} proposals  (${r.source_doc})`);
      }
      lines.push(``);
      lines.push(`Store safety:`);
      lines.push(`  path:           ${result.validation_store_path}`);
      lines.push(`  mtime before:   ${result.validation_store_mtime_before ?? "(store does not exist)"}`);
      lines.push(`  mtime after:    ${result.validation_store_mtime_after ?? "(store does not exist)"}`);
      lines.push(`  unchanged:      ${result.validation_store_mtime_unchanged ? "YES (safe)" : "⚠ NO (safety violation)"}`);
      if (result.warnings.length > 0) {
        lines.push(``);
        lines.push(`Warnings:`);
      ...
      return structuredResult(lines.join("\n"), {
        schema: SCHEMA,
        data: result,
        provenance: provenance("local", "local:auto-validate-from-external-anchors", VERSION, {
          citation:
            "Proposal-only auto-validation against methods-paper external anchors (§3.6 / §3.7.3 / §3.11). Never mutates the v1.0-gate validation-resolutions store. v0.64.0. T1-B from post-JOHD upgrade plan.",
        }),
        warnings: result.warnings.length > 0 ? result.warnings : undefined,
      });
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      return structuredResult(`auto_validate_from_resolutions error: ${msg}`, {
        schema: SCHEMA,
        data: {
          mode: "propose" as const,
          proposal_file_path: "",
          proposals: [] as never[],
          proposed_positives: 0,
          proposed_negatives: 0,
          rules_applied: [] as never[],
          validation_store_path: "",
          validation_store_mtime_before: null,
          validation_store_mtime_after: null,
          validation_store_mtime_unchanged: true,
          warnings: [msg],
        },
        provenance: provenance("local", "local:auto-validate-from-external-anchors", VERSION),
        warnings: [msg],
...
// ─── v0.65.0 — cdli_ebl_crosswalk (bidirectional CDLI ↔ eBL ID mapping) ───

server.registerTool(
  "cdli_ebl_crosswalk",
  {
    description:
      "Bidirectional CDLI ↔ eBL ID crosswalk. Accepts a CDLI P-number ('P396240'), bare CDLI integer id ('396240'), or eBL museum number ('K.5896', 'BM 47463', 'Ki.1904-10-9.78'). Auto-detects the input type, normalizes museum numbers (space → dot; leading zeros stripped; internal dashes preserved for date-style numbers), and returns all matching pairs. Each match is tagged 'native' (link came from a typed field on either upstream — eBL's externalNumbers.cdliNumber or CDLI's external_resources[abbrev=eBL]) or 'inferred_via_museum_number' (CDLI's external_resources lacked the eBL row, so we parsed CDLI's museum_no string and probed eBL live; a ' + '-separated join expression like 'BM 047463 + BM 049124' expands into multiple matches). Returns matches=[] with a warning when neither side has a record. v0.65.0.",
    inputSchema: {
      id: z
        .string()
        .min(1)
        .describe(
          "CDLI P-number (e.g. 'P396240'), bare CDLI integer id (e.g. '396240'), or eBL museum number (e.g. 'K.5896', 'BM.47463', 'BM 47463').",
        ),
    },
  },
  async ({ id }) => {
    const SCHEMA = schemaId("cdli_ebl_crosswalk");
    try {
      const result = await cdliEblCrosswalk({ id, userAgent: USER_AGENT });
      const lines: string[] = [];
      lines.push(
        `cdli_ebl_crosswalk("${result.query.input_id}") — detected_type=${result.query.detected_type}`,
      );
      lines.push("");
      if (result.matches.length === 0) {
        ...
        for (const m of result.matches) {
          const cdliBit =
            m.cdli_p_number !== null
              ? `${m.cdli_p_number} (id=${m.cdli_integer_id})`
              : m.cdli_integer_id !== null
                ? `id=${m.cdli_integer_id}`
                : "(no CDLI link)";
          lines.push(
            `  ${m.ebl_id}  ↔  ${cdliBit}  [${m.confidence}]`,
          );
        }
      }
      if (result.warnings.length > 0) {
      ...
      // Provenance endpoint summarizes both upstreams. We pick the dominant
      // side based on detected input.
      const endpoint =
        result.query.detected_type === "ebl_museum_number"
          ? `${URLS.EBL_BASE}/fragments/${encodeURIComponent(id)}`
          : `${URLS.CDLI_BASE}/artifacts/${id}`;
      const source =
        result.query.detected_type === "ebl_museum_number" ? "eBL" : "CDLI";
      return structuredResult(lines.join("\n"), {
        schema: SCHEMA,
        data: result,
        provenance: provenance(source, endpoint, VERSION, {
          citation:
            "Bidirectional CDLI/eBL ID crosswalk — typed cross-references from both upstreams, with museum_no string fallback when CDLI lacks the reciprocal eBL row. v0.65.0.",
        }),
        warnings: result.warnings.length > 0 ? result.warnings : undefined,
      });
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      return structuredResult(`cdli_ebl_crosswalk error: ${msg}`, {
        schema: SCHEMA,
        data: {
          query: { input_id: id, detected_type: "ebl_museum_number" as const },
          matches: [] as never[],
          warnings: [msg],
        },
        provenance: provenance("local", "local:cdli-ebl-crosswalk-error", VERSION),
        warnings: [msg],
      });
    }
  },
...
// ─── v0.66.0 — detect_bilingual_tablet (Sumerian/Akkadian classifier) ────

server.registerTool(
  "detect_bilingual_tablet",
  {
    description:
      "Live single-tablet Sumerian/Akkadian bilingual classifier. Hits eBL /fragments/{museum_number} directly (no cache dependency) and walks text.lines[].content[] per-Word language tags. The load-bearing discriminator is per-Word .language: eBL's lemmatizer correctly tags a sumerogram (EN₂ → šiptu, MUŠEN → iṣṣūru, etc.) with language=AKKADIAN even when the surface looks Sumerian, which separates a true bilingual line (some Words tagged SUMERIAN, others AKKADIAN) from 'Akkadian written with sumerograms' (every Word tagged AKKADIAN despite Sumerian-looking surface). Returns one of seven classifications: interlinear_bilingual, alternating_line_bilingual, akkadian_with_sumerograms, monolingual_sumerian, monolingual_akkadian, insufficient_data, uncertain. The classifier is intentionally conservative — when the signal is ambiguous it returns 'uncertain'/'insufficient_data' rather than a false-positive bilingual call. Optional genre prior is stamped from the local fragment-metadata cache for reporting but never gates the decision. v0.66.0.",
    inputSchema: {
      tablet_id: z
        .string()
        .min(1)
        .describe(
          "eBL museum number (dotted form, e.g. 'K.4178', 'K.133', 'K.2798').",
        ),
    },
  },
  async ({ tablet_id }) => {
    const SCHEMA = schemaId("detect_bilingual_tablet");
    try {
      // Genre prior is informational — pulled from the local fragment-metadata
      // cache when available, never gates the live classification.
      const md = _getFragmentMetadataForImages(tablet_id);
      const { genre_prior, genre_path } = resolveGenrePrior(md?.genres ?? null);
      const result = await detectBilingualTablet({
        tabletId: tablet_id,
        userAgent: USER_AGENT,
        genreHint: genre_prior,
        genrePath: genre_path,
      });
      const lines: string[] = [];
      lines.push(
        `detect_bilingual_tablet("${result.tablet_id}") — classification=${result.classification} (is_bilingual=${result.is_bilingual})`,
      );
      lines.push("");
      lines.push(
        `Signal: text_lines=${result.signal.text_line_count} | sum_only=${result.signal.sumerian_only_line_count} akk_only=${result.signal.akkadian_only_line_count} mixed=${result.signal.mixed_token_line_count}`,
      );
      lines.push(
        `        sum_tokens=${result.signal.sumerian_token_count} (${(result.signal.sumerian_token_share * 100).toFixed(1)}%) | akk_tokens=${result.signal.akkadian_token_count} (${(result.signal.akkadian_token_share * 100).toFixed(1)}%)`,
      );
      lines.push(
        `        alternation_pattern=${result.signal.alternation_pattern} | language_shifts=${result.signal.language_shift_count}`,
      );
      lines.push(
        `        genre_prior=${result.signal.genre_prior}${result.signal.genre_path ? ` (${result.signal.genre_path})` : ""}`,
      );
      ...
      const endpoint = `${URLS.EBL_BASE}/fragments/${encodeURIComponent(tablet_id)}`;
      return structuredResult(lines.join("\n"), {
        schema: SCHEMA,
        data: result,
        provenance: provenance("eBL", endpoint, VERSION, {
          citation:
            "Per-Word language tags from eBL fragment lemmatizer — distinguishes true Sumerian/Akkadian bilinguals from Akkadian-with-sumerograms. v0.66.0.",
        }),
        warnings: result.warnings.length > 0 ? result.warnings : undefined,
      });
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      return structuredResult(`detect_bilingual_tablet error: ${msg}`, {
        schema: SCHEMA,
        data: {
          tablet_id,
          is_bilingual: false,
          ...
          signal: {
            ...
            alternation_pattern: "none" as const,
            genre_prior: "unknown" as const,
            genre_path: null,
          },
          reasoning: [`error: ${msg}`],
          warnings: [msg],
        },
        provenance: provenance("local", "local:detect-bilingual-tablet-error", VERSION),
        warnings: [msg],
      });
    }
  },
...
// ─── v0.66.0 — find_bilingual_tablets (cache-backed corpus surface) ──────

server.registerTool(
  "find_bilingual_tablets",
  {
    description:
      "Cache-backed corpus-wide bilingual tablet surface. Reads ~/.cache/cuneiform-mcp/bilingual-index.json (built by scripts/build-bilingual-index.mjs) and returns ranked Sumerian/Akkadian bilingual candidates by composite confidence (threshold-gap × token-share-balance). The cache builder pre-filters the corpus by a 12-entry bilingual-genre prior (Lugal-e, Angim, Udugḫul, Mīs pî, Šurpu, Diri, Ura, An=Anum, Izi, Lamentations, Šuʾila, Marduk's Address to the Demons) cutting the API budget from ~36K → ~4,370 tablets (~88% reduction). If the cache is absent or empty the tool returns tablets=[] with a clear build-instruction warning rather than crashing. Filter by classification_filter (e.g. ['interlinear_bilingual']) or min_confidence to narrow the surface. v0.66.0.",
    inputSchema: {
      top_k: z
        .number()
        .int()
      ...
      classification_filter: z
        ...
        .optional()
        .describe(
          "Restrict results to these classifications. Default: ['interlinear_bilingual','alternating_line_bilingual'] (both true-bilingual sub-types).",
        ),
      min_confidence: z
        .number()
        .min(0)
        ...
        .optional()
        .describe(
          "Minimum composite confidence in [0,1]. Default 0 (return all matches in the cache).",
        ),
    },
  },
  async ({ top_k, classification_filter, min_confidence }) => {
    const SCHEMA = schemaId("find_bilingual_tablets");
    try {
      const result = findBilingualTablets({
        topK: top_k,
        classificationFilter: classification_filter,
        minConfidence: min_confidence,
      });
      const lines: string[] = [];
      lines.push(
        `find_bilingual_tablets — ${result.tablets.length} match(es) over ${result.cache_stats.tablets_in_cache} cached tablets (${result.cache_stats.tablets_classified_bilingual} classified bilingual)`,
      );
      if (result.cache_stats.cache_built_at) {
        lines.push(`  cache built_at: ${result.cache_stats.cache_built_at}`);
      }
      lines.push("");
      if (result.tablets.length === 0) {
        ...
        lines.push(
          `Top ${Math.min(result.tablets.length, 20)} (ranked by composite confidence):`,
        );
        for (const t of result.tablets.slice(0, 20)) {
          lines.push(
            `  ${t.tablet_id}  [${t.classification}]  conf=${t.confidence.toFixed(3)}  sum_share=${(t.sumerian_token_share * 100).toFixed(0)}% akk_share=${(t.akkadian_token_share * 100).toFixed(0)}%  lines=${t.text_line_count}  ${t.genre_path ?? "(no genre)"}`,
          );
        }
      }
      if (result.warnings.length > 0) {
      ...
      return structuredResult(lines.join("\n"), {
        ...
        provenance: provenance(
          "local",
          `local:bilingual-index:${result.cache_stats.cache_path}`,
          VERSION,
          {
            citation:
              "Bilingual classifications sourced from eBL /fragments/{id} per-Word language tags, cached via scripts/build-bilingual-index.mjs. v0.66.0.",
          },
        ),
        warnings: result.warnings.length > 0 ? result.warnings : undefined,
      });
      ...
      const msg = e instanceof Error ? e.message : String(e);
      return structuredResult(`find_bilingual_tablets error: ${msg}`, {
        schema: SCHEMA,
        data: {
          query: {
            top_k: top_k ?? 50,
            classification_filter: classification_filter ?? null,
        ...
        provenance: provenance("local", "local:find-bilingual-tablets-error", VERSION),
        warnings: [msg],
      });
    }
  },
...
// ─── v0.63.0 — diff_corpus_versions (cache-snapshot delta) ────────────────

server.registerTool(
  "diff_corpus_versions",
  {
    description:
      "Read-only delta between two content-hash manifests of ~/.cache/cuneiform-mcp/. Manifests are produced by scripts/snapshot-cache.mjs (walks the cache dir, computes SHA-256 per file). Returns added/removed/changed file lists + summary (file counts, signed bytes_delta). The diff tool itself NEVER writes anything under the cache dir — manifests live wherever the caller puts them. Use cases: (1) reproducibility audits — pin methods-paper claims to a specific manifest hash; (2) enrichment-burst impact — snapshot before + after an eBL metadata enrichment, surface which fragment IDs moved; (3) v0.20-alpha-style burst audits — verify nothing was overwritten unintentionally. v0.63.0. T2-A from post-JOHD upgrade plan.",
    inputSchema: {
      old_manifest_path: z.string().describe("Absolute path to the older manifest JSON."),
      new_manifest_path: z.string().describe("Absolute path to the newer manifest JSON."),
    },
  },
  async ({ old_manifest_path, new_manifest_path }) => {
    const SCHEMA = schemaId("diff_corpus_versions");
    try {
      const result = diffCorpusVersions({
        oldManifestPath: old_manifest_path,
        newManifestPath: new_manifest_path,
      });
      const lines: string[] = [
        `Cache-snapshot delta`,
        `  old: ${result.summary.old_manifest_at}  (${result.summary.files_in_old} files)`,
        `  new: ${result.summary.new_manifest_at}  (${result.summary.files_in_new} files)`,
        ``,
        `  +${result.summary.added_count} added · -${result.summary.removed_count} removed · ~${result.summary.changed_count} changed · ${result.summary.bytes_delta >= 0 ? "+" : ""}${result.summary.bytes_delta} bytes`,
        ``,
      ];
      const previewAdded = result.files_added.slice(0, 10);
      const previewRemoved = result.files_removed.slice(0, 10);
      const previewChanged = result.files_changed.slice(0, 10);
      if (previewAdded.length > 0) {
        lines.push(`Added (first ${previewAdded.length} of ${result.files_added.length}):`);
        for (const f of previewAdded) lines.push(`  + ${f.path} (${f.size_bytes} B)`);
        lines.push(``);
      }
      if (previewRemoved.length > 0) {
        lines.push(`Removed (first ${previewRemoved.length} of ${result.files_removed.length}):`);
        for (const f of previewRemoved) lines.push(`  - ${f.path} (${f.size_bytes} B)`);
        lines.push(``);
      }
      if (previewChanged.length > 0) {
        lines.push(`Changed (first ${previewChanged.length} of ${result.files_changed.length}):`);
        for (const c of previewChanged) {
          lines.push(`  ~ ${c.path}  ${c.old_hash.slice(0, 8)}…→${c.new_hash.slice(0, 8)}…  ${c.old_size}→${c.new_size} B`);
        }
      }
      return structuredResult(lines.join("\n"), {
        schema: SCHEMA,
        data: result,
        provenance: provenance("local", "local:cache-snapshot-diff", VERSION, {
          citation:
            "Read-only delta between two content-hash manifests of ~/.cache/cuneiform-mcp/. v0.63.0. T2-A from post-JOHD upgrade plan.",
        }),
      });
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      return structuredResult(`diff_corpus_versions error: ${msg}`, {
        schema: SCHEMA,
        data: {
          files_added: [] as never[],
          files_removed: [] as never[],
          files_changed: [] as never[],
          summary: {
            old_manifest_at: "",
            new_manifest_at: "",
            files_in_old: 0,
...
// ─── v0.62.0 — export_session (session ring buffer + snapshot) ────────────

server.registerTool(
  "export_session",
  {
    description:
      "Snapshot the in-process session ring buffer to ~/.cache/cuneiform-mcp/sessions/<iso-ts>.{json,md}. Every tool call's structuredContent envelope is captured automatically (capped at 200 envelopes per session by default, configurable via CUNEIFORM_MCP_SESSION_BUFFER). Returns the snapshot paths + envelope count + oldest/newest timestamps. Use when bundling a research session for reproducibility — paper drafts, reviewer responses, collaborator handoffs — without needing to re-run every tool to recapture the data. The JSON snapshot preserves the full envelope (typed data + provenance + warnings); the Markdown snapshot renders one section per envelope with the rendered text block. v0.62.0. T1-A → T1-C from the post-JOHD upgrade plan.",
    inputSchema: {},
  },
  async () => {
    const SCHEMA = schemaId("export_session");
    try {
      ...
      const lines: string[] = [
        ...
        ``,
        `  JSON: ${result.path_json}`,
        `  MD:   ${result.path_md}`,
        ``,
        `  Envelopes: ${result.envelope_count} (buffer capacity ${result.buffer_capacity}; lifetime records ${stats.record_count})`,
      ];
      if (result.oldest_timestamp) {
        lines.push(`  Oldest: ${result.oldest_timestamp}`);
        lines.push(`  Newest: ${result.newest_timestamp}`);
      } else {
        lines.push(`  (buffer empty — no envelopes recorded yet this session)`);
      }
      return structuredResult(lines.join("\n"), {
        schema: SCHEMA,
        data: result,
        provenance: provenance("local", "local:session-ring-buffer-snapshot", VERSION, {
          citation:
            "Session ring buffer snapshot to ~/.cache/cuneiform-mcp/sessions/. v0.62.0. T1-C from post-JOHD upgrade plan.",
        }),
      });
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      return structuredResult(`export_session error: ${msg}`, {
        schema: SCHEMA,
        data: {
          path_json: "",
          path_md: "",
...
// ─── v0.30.0 — restore_lacuna_semantic (sign2vec-augmented lacuna) ────────

server.registerTool(
  "restore_lacuna_semantic",
  {
    description:
      "Single-position lacuna restoration using a joint score combining (a) the v0.18.0 bigram-context heuristic and (b) the v0.23 sign2vec semantic prior derived from the surrounding visible signs. α∈[0,1] interpolates: α=1 = pure bigram (v0.18.0 baseline), α=0 = pure sign2vec semantic, α=0.5 = balanced (default). EMPIRICAL: on a 10-tablet sample, α=0 and α=1 disagree on the top-1 prediction in 9/10 cases — the semantic axis provides STRONGLY INDEPENDENT signal, not redundant re-ranking. Sample case: 1879,0708.118 position 20, ground truth ABZ52 — joint top-1 recovers ABZ52, pure sign2vec would have picked ABZ570 (wrong), pure bigram correctly picked ABZ52. The semantic axis adds complementary not corrective signal at this case. Methods paper §3.17.",
    inputSchema: {
      tablet_id: z.string().optional().describe("Museum number of the tablet. Provide this OR signs."),
      signs: z.string().optional().describe("Raw signs string (X/x/? mark damage). Use if the tablet isn't in the corpus index."),
      lacuna_position: z.number().int().min(0).describe("Index of the missing sign in the tablet's tokenized signs array."),
      top_k: z.number().int().min(1).max(50).optional().describe("Top-K candidates to return. Default 10."),
      alpha: z.number().min(0).max(1).optional().describe("Interpolation knob between bigram (α=1) and sign2vec (α=0) axes. Default 0.5."),
    },
  },
  async ({ tablet_id, signs, lacuna_position, top_k, alpha }) => {
    const SCHEMA = schemaId("restore_lacuna_semantic");
    try {
      const result = restoreLacunaSemantic({ tablet_id, signs, lacuna_position, top_k, alpha });
      const lines: string[] = [
        `Tablet: ${result.tablet_id ?? "(inline signs)"}  ·  position: ${result.lacuna_position}  ·  α=${result.alpha.toFixed(2)}`,
        `Surrounding: left2=${result.surrounding_signs.left2 ?? "?"}  left1=${result.surrounding_signs.left1 ?? "?"}  →  [?]  ←  right1=${result.surrounding_signs.right1 ?? "?"}  right2=${result.surrounding_signs.right2 ?? "?"}`,
        ``,
        `Top-${result.predictions.length} predictions:`,
      ];
      for (const p of result.predictions) {
        lines.push(`  ${p.rank_by_joint.toString().padStart(2)}. ${p.sign.padEnd(12)}  joint=${p.joint_score.toFixed(4)}  bigram=${p.bigram_score.toFixed(4)}  sign2vec=${p.sign2vec_score.toFixed(4)}`);
      }
      lines.push(``);
      lines.push(`Ablation:`);
      lines.push(`  pure_bigram_top1:    ${result.ablation.pure_bigram_top1 ?? "—"}`);
      lines.push(`  pure_sign2vec_top1:  ${result.ablation.pure_sign2vec_top1 ?? "—"}`);
      lines.push(`  joint_top1:          ${result.ablation.joint_top1 ?? "—"}`);
      lines.push(`  agreement:           ${result.ablation.agreement}`);
      if (result.warnings.length > 0) lines.push(`Warnings: ${result.warnings.join("; ")}`);
      return structuredResult(lines.join("\n"), {
        schema: SCHEMA,
        data: result,
        provenance: provenance("local", "local:lacuna-restoration-bigram-sign2vec-joint", VERSION, {
          citation: "Single-position lacuna restoration via joint bigram-context (v0.18.0) + sign2vec semantic-prior (v0.23) score. Methods paper §3.17. v0.30.0.",
        }),
        warnings: result.warnings.length > 0 ? result.warnings : undefined,
      });
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      return structuredResult(`restore_lacuna_semantic error: ${msg}`, {
        schema: SCHEMA,
        data: {
          tablet_id: tablet_id ?? null, lacuna_position, alpha: alpha ?? 0.5,
          embedding_stats: { loaded: false, embedding_dim: 0, total_signs_indexed: 0, signs_present_in_context: 0 },
          surrounding_signs: { left2: null, left1: null, right1: null, right2: null },
          predictions: [] as never[],
          ablation: { pure_bigram_top1: null, pure_sign2vec_top1: null, joint_top1: null, agreement: "all_agree" },
          warnings: [msg],
        },
        provenance: provenance("local", "local:lacuna-restoration-bigram-sign2vec-joint", VERSION),
        warnings: [msg],
...
// ─── v0.32.0 — identify_composition (composition assignment) ─────────────

server.registerTool(
  "identify_composition",
  {
    description:
      "Composition assignment for a query tablet. Returns ranked candidate compositions (Mīs pî, Šurpu, Udug-ḫul, Bīt salāʾ mê, āšipūtu curriculum, ...) from the methods-paper-anchored registry, scored on a joint of (a) chunk-overlap with each composition's exemplar pool (length-20 chunk-hash index, §3.10) and (b) sign2vec sign-centroid cosine (§3.12). The query is filtered out of its own exemplar pool so the score reflects sibling overlap rather than degenerate self-match. Compositions tagged composition_type='curriculum' (e.g. KAR-44 āšipūtu) are EXPECTED to fire alongside specific-composition hits — a tablet is both a Mīs pî manuscript AND a member of the āšipūtu curriculum. Confidence weighting normalizes over APPLICABLE axes only, so a missing cache penalizes coverage rather than the score. Methods §3.19, claim 39.",
    inputSchema: {
      tablet_id: z.string().min(1).describe("Museum number to identify (must exist in the signs cache)."),
      top_k: z.number().int().min(1).max(20).optional().describe("Number of candidate compositions to return (default 5)."),
      min_confidence: z.number().min(0).max(1).optional().describe("Confidence floor. Default 0 (return all)."),
      chunk_overlap_weight: z.number().min(0).max(1).optional().describe("Weight on chunk-overlap axis. Default 0.6."),
      sign2vec_centroid_weight: z.number().min(0).max(1).optional().describe("Weight on sign2vec-centroid axis. Default 0.4."),
    },
  },
  async ({ tablet_id, top_k, min_confidence, chunk_overlap_weight, sign2vec_centroid_weight }) => {
    const SCHEMA = schemaId("identify_composition");
    try {
      const result = identifyComposition({
        tabletId: tablet_id,
        topK: top_k,
        minConfidence: min_confidence,
        axisWeights: {
          chunk_overlap: chunk_overlap_weight,
          sign2vec_centroid: sign2vec_centroid_weight,
        },
      });
      const lines: string[] = [
        `Query: ${result.query.tablet_id}  ·  sign_count=${result.query.sign_count ?? "n/a"}  ·  period=${result.query.period ?? "n/a"}  ·  genre=${result.query.primary_genre ?? "n/a"}`,
        `Axes: chunk_overlap=${result.index_stats.chunk_index_loaded ? "on" : "off"}  ·  sign2vec_centroid=${result.index_stats.sign_embeddings_loaded ? "on" : "off"}  ·  query_embedded_signs=${result.index_stats.query_signs_with_embedding}`,
        `Compositions considered: ${result.compositions_considered}`,
        ``,
        `Candidates (top ${result.candidates.length}):`,
        ``,
      ];
      for (const [i, c] of result.candidates.entries()) {
        lines.push(`${i + 1}. ${c.composition_name} [${c.composition_type}]  ·  confidence=${c.confidence.toFixed(3)}`);
        lines.push(`     chunk: raw=${c.axis_scores.chunk_overlap.raw} norm=${c.axis_scores.chunk_overlap.normalized.toFixed(3)}  ·  sign2vec: raw=${c.axis_scores.sign2vec_centroid.raw.toFixed(3)} norm=${c.axis_scores.sign2vec_centroid.normalized.toFixed(3)}`);
      ...
      return structuredResult(lines.join("\n"), {
        schema: SCHEMA,
        data: result,
        provenance: provenance("local", "local:identify-composition-from-registry", VERSION, {
          citation:
            "Composition assignment via joint chunk-overlap + sign2vec-centroid scoring against methods-paper-anchored exemplar registry. v0.32.0, §3.19.",
        }),
        warnings: result.warnings.length > 0 ? result.warnings : undefined,
      });
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      return structuredResult(`identify_composition error: ${msg}`, {
        schema: SCHEMA,
        data: {
          query: { tablet_id, sign_count: null, period: null, primary_genre: null },
          candidates: [] as never[],
          compositions_considered: 0,
          index_stats: { chunk_index_loaded: false, sign_embeddings_loaded: false, signs_cache_loaded: false, query_signs_with_embedding: 0 },
          warnings: [msg],
        },
        provenance: provenance("local", "local:identify-composition-from-registry", VERSION),
        warnings: [msg],
...
// ─── v0.33.0 — build_stemma_with_rooting (rooted recension trees) ────────

server.registerTool(
  "build_stemma_with_rooting",
  {
    description:
      "Extends v0.22 `build_canonical_recension_tree` (which produces UNROOTED neighbor-joining trees with a trifurcation at the algorithmic root) by re-rooting the stemma at a witness chosen via one of three heuristics: 'earliest_period' (Mesopotamian-canonical OB→MB→MA→NA→NB→LB ordering — earlier periods are closer to the archetype), 'most_chunk_hosts' (graph-theoretic coverage centroid — the witness with the broadest corpus reach is treated as the structural center), or 'outgroup_witness' (caller-specified, useful when an OB forerunner is identified externally). Returns both the original unrooted Newick AND the rooted Newick + edge list + root-choice rationale. Methods §3.20, claim 40.",
    inputSchema: {
      seed_tablet_id: z.string().min(1).describe("Museum number of a known manuscript of the composition (e.g. 'K.5896' for Mīs pî). The cluster is BFS-expanded from this seed exactly as in v0.22."),
      rooting_mode: z.enum(["earliest_period", "most_chunk_hosts", "outgroup_witness"]).describe("Rooting heuristic. 'earliest_period' uses the Mesopotamian period ordering. 'most_chunk_hosts' picks the witness with the broadest corpus reach. 'outgroup_witness' requires `outgroup_witness` and roots at that specific tablet."),
      outgroup_witness: z.string().optional().describe("Required when rooting_mode='outgroup_witness'. Must be one of the witnesses in the cluster (else null root + warning)."),
      max_witnesses: z.number().int().min(2).max(200).optional().describe("Cluster size cap. Default 50, mirroring v0.22."),
      min_pairwise_chunks: z.number().int().min(1).optional().describe("Minimum shared chunks for a witness pair to enter the distance matrix. Default 3."),
      algorithm: z.enum(["neighbor_joining", "upgma"]).optional().describe("Base tree algorithm. Default neighbor_joining."),
    },
  },
  async ({ seed_tablet_id, rooting_mode, outgroup_witness, max_witnesses, min_pairwise_chunks, algorithm }) => {
    const SCHEMA = schemaId("build_stemma_with_rooting");
    try {
      const result = buildStemmaWithRooting({
        seedTabletId: seed_tablet_id,
        rootingMode: rooting_mode as RootingMode,
        outgroupWitness: outgroup_witness,
        maxWitnesses: max_witnesses,
        minPairwiseChunks: min_pairwise_chunks,
        algorithm,
      });
      const lines: string[] = [
        `Seed: ${result.composition_seed}  ·  algorithm: ${result.algorithm}  ·  witnesses: ${result.witnesses.length}`,
        `Rooting mode: ${result.rooting.mode}  ·  root: ${result.rooting.root_witness ?? "(none)"}  ·  candidates: ${result.rooting.candidates_considered}`,
        `Rationale: ${result.rooting.root_choice_rationale}`,
        ``,
        `Unrooted Newick:`,
        result.unrooted_newick || "(empty — see warnings)",
        ``,
        `Rooted Newick:`,
        result.rooted_newick || "(empty — see warnings)",
      ];
      if (result.warnings.length > 0) {
        lines.push(``);
        lines.push(`Warnings: ${result.warnings.join("; ")}`);
      ...
      return structuredResult(lines.join("\n"), {
        schema: SCHEMA,
        data: result,
        provenance: provenance("local", "local:rooted-stemma-from-v022-nj", VERSION, {
          citation:
            "Re-rooting of v0.22 NJ stemma via earliest-period / coverage-centroid / outgroup-witness heuristics. v0.33.0, §3.20.",
        }),
        warnings: result.warnings.length > 0 ? result.warnings : undefined,
      });
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      return structuredResult(`build_stemma_with_rooting error: ${msg}`, {
        schema: SCHEMA,
        data: {
          composition_seed: seed_tablet_id,
          algorithm: algorithm ?? "neighbor_joining",
          rooting: { mode: rooting_mode, root_witness: null, root_choice_rationale: msg, candidates_considered: 0 },
          witnesses: [] as never[],
          unrooted_newick: "",
          rooted_newick: "",
          rooted_tree_edges: [] as never[],
          distance_matrix: [] as never[],
          warnings: [msg],
        },
        provenance: provenance("local", "local:rooted-stemma-from-v022-nj", VERSION),
...
// ─── v0.34.0 — score_tablet_completeness (fragment-vs-composition gap) ───

server.registerTool(
  "score_tablet_completeness",
  {
    description:
      "Given a fragment, estimate what fraction of the original composition is preserved. Two complementary metrics: (1) sign_count_ratio = query.sign_count / largest_exemplar.sign_count, capped at 1.0 — a size-proxy for how much physical text survives; (2) chunk_coverage_ratio = |query_chunks ∩ composition_canonical_chunks| / |composition_canonical_chunks|, where canonical_chunks = length-20 hashes appearing in ≥2 of the composition's registry exemplars. The canonical-chunk set is the structural backbone, stripped of single-witness noise. Composition is resolved via composition_id (explicit) or inferred via identify_composition's top candidate (when confidence ≥ fallback_min_confidence, default 0.3). Useful for prioritizing reconstruction effort: low chunk_coverage + high sign_count_ratio = surviving text from underrepresented sections; high chunk_coverage + low sign_count = condensed/abbreviated witness. Methods §3.21, claim 41.",
    inputSchema: {
      tablet_id: z.string().min(1).describe("Museum number of the fragment to score."),
      composition_id: z.string().optional().describe("Optional explicit composition (one of: mis_pi, surpu, udug_hul, bit_sala_me, asiputu_kar44). If omitted, identify_composition is called and its top candidate used."),
      fallback_min_confidence: z.number().min(0).max(1).optional().describe("When composition is inferred, the top-candidate confidence floor below which the result is marked 'unresolved' instead of 'inferred'. Default 0.3."),
      include_chunk_lists: z.boolean().optional().describe("If true, populate preserved_chunk_hashes + missing_chunk_hashes arrays. Default false — keeps response small."),
    },
  },
  async ({ tablet_id, composition_id, fallback_min_confidence, include_chunk_lists }) => {
    const SCHEMA = schemaId("score_tablet_completeness");
    try {
      const result = scoreTabletCompleteness({
        tabletId: tablet_id,
        compositionId: composition_id,
        fallbackMinConfidence: fallback_min_confidence,
        includeChunkLists: include_chunk_lists,
      });
      const m = result.metrics;
      const c = result.composition;
      const lines: string[] = [
        `Query: ${result.query_tablet_id}  ·  sign_count=${m.query_sign_count ?? "n/a"}  ·  lacuna_density=${m.lacuna_density?.toFixed(3) ?? "n/a"}`,
        `Composition: ${c.composition_name ?? "(unresolved)"} [${c.source}]${c.inferred_confidence !== null ? `  conf=${c.inferred_confidence.toFixed(3)}` : ""}`,
        `   rationale: ${c.inferred_rationale ?? "n/a"}`,
        ``,
        `Completeness metrics:`,
        `   sign_count_ratio:    ${m.sign_count_ratio?.toFixed(3) ?? "n/a"}  (${m.query_sign_count ?? "?"} / ${m.largest_exemplar_sign_count ?? "?"} from ${m.largest_exemplar_id ?? "?"})`,
        `   chunk_coverage:      ${m.chunk_coverage_ratio?.toFixed(3) ?? "n/a"}  (${m.chunks_hosted_count} / ${m.canonical_chunks_count} canonical chunks hosted)`,
        ``,
        `Index: chunks=${result.index_stats.chunk_index_loaded ? "on" : "off"}  signs=${result.index_stats.signs_cache_loaded ? "on" : "off"}  ·  exemplars_with_chunks=${result.index_stats.exemplars_with_chunks}  exemplars_with_signs=${result.index_stats.exemplars_with_signs}`,
      ];
      if (result.warnings.length > 0) {
        lines.push(``);
        lines.push(`Warnings: ${result.warnings.join("; ")}`);
      ...
      return structuredResult(lines.join("\n"), {
        schema: SCHEMA,
        data: result,
        provenance: provenance("local", "local:completeness-vs-canonical-chunks", VERSION, {
          citation:
            "Fragment-vs-composition completeness estimator. sign_count_ratio + chunk_coverage against canonical-chunk backbone (≥2-exemplar hashes). v0.34.0, §3.21.",
        }),
        warnings: result.warnings.length > 0 ? result.warnings : undefined,
      });
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      return structuredResult(`score_tablet_completeness error: ${msg}`, {
        schema: SCHEMA,
        data: {
          query_tablet_id: tablet_id,
          composition: { source: "unresolved", composition_id: null, composition_name: null, inferred_confidence: null, inferred_rationale: msg },
          metrics: {
            sign_count_ratio: null,
            chunk_coverage_ratio: null,
            chunks_hosted_count: 0,
            canonical_chunks_count: 0,
            query_sign_count: null,
            largest_exemplar_id: null,
            largest_exemplar_sign_count: null,
            lacuna_density: null,
          },
          preserved_chunk_hashes: [] as never[],
          missing_chunk_hashes: [] as never[],
          index_stats: { chunk_index_loaded: false, signs_cache_loaded: false, exemplars_with_chunks: 0, exemplars_with_signs: 0 },
          warnings: [msg],
        },
        provenance: provenance("local", "local:completeness-vs-canonical-chunks", VERSION),
        warnings: [msg],
...
// ─── v0.35.0 — find_composition_lineage (transmission tracer) ────────────

server.registerTool(
  "find_composition_lineage",
  {
    description:
      "Trace a composition's transmission across periods and ateliers. Composes v0.20 chunk index + v0.22 stemma-BFS + v0.32 composition registry + fragment metadata (period + provenance). Given a composition (composition_id or seed_tablet_id), expand the witness cluster, bucket witnesses by (period × provenance) into 'transmission nodes,' draw chunk-sharing edges between adjacent nodes (≥min_edge_chunks), and surface 'bridge witnesses' whose chunks span ≥2 nodes. Output answers: where was this composition copied, when, and which witnesses chained transmission across atelier or period boundaries? Composition resolution mirrors score_tablet_completeness: explicit composition_id or identify_composition's top candidate (above fallback_min_confidence). Methods §3.22, claim 42.",
    inputSchema: {
      composition_id: z.string().optional().describe("Optional explicit composition (mis_pi / surpu / udug_hul / bit_sala_me / asiputu_kar44). When omitted, seed_tablet_id's top identify_composition candidate is used."),
      seed_tablet_id: z.string().optional().describe("Optional seed tablet for BFS expansion. When omitted, composition's first registry exemplar is used. One of {composition_id, seed_tablet_id} is required."),
      max_witnesses: z.number().int().min(2).max(200).optional().describe("Witness-cluster cap. Default 50."),
      min_pairwise_chunks: z.number().int().min(1).optional().describe("Minimum shared chunks for witness-pair inclusion in the v0.22 BFS. Default 3."),
      min_edge_chunks: z.number().int().min(1).optional().describe("Minimum shared chunks between two (period, provenance) nodes for an edge to be emitted. Default 5."),
      fallback_min_confidence: z.number().min(0).max(1).optional().describe("When composition is inferred via seed, top-candidate confidence floor below which result is unresolved. Default 0.3."),
    },
  },
  async ({ composition_id, seed_tablet_id, max_witnesses, min_pairwise_chunks, min_edge_chunks, fallback_min_confidence }) => {
    const SCHEMA = schemaId("find_composition_lineage");
    try {
      const result = findCompositionLineage({
        compositionId: composition_id,
        seedTabletId: seed_tablet_id,
        maxWitnesses: max_witnesses,
        minPairwiseChunks: min_pairwise_chunks,
        minEdgeChunks: min_edge_chunks,
        fallbackMinConfidence: fallback_min_confidence,
      });
      const s = result.diffusion_summary;
      const lines: string[] = [
        `Composition: ${result.composition.composition_name ?? "(unresolved)"} [${result.composition.source}]`,
        `Seed: ${result.query.seed_tablet_id}  ·  witnesses=${s.n_witnesses}  ·  nodes=${s.n_nodes}  ·  edges=${s.n_edges}`,
        `Periods: ${s.earliest_period ?? "?"} → ${s.latest_period ?? "?"}  (${s.n_distinct_periods} distinct)  ·  provenances: ${s.n_distinct_provenances}`,
        `Cross-period edges: ${s.n_cross_period_edges}  ·  cross-provenance edges: ${s.n_cross_provenance_edges}  ·  bridge witnesses: ${s.n_bridge_witnesses}`,
        ``,
        `Transmission nodes (top 10):`,
      ];
      for (const node of result.transmission_nodes.slice(0, 10)) {
        lines.push(`  ${node.node_id}  members=${node.member_tablets.length}  signs=${node.total_sign_count}  hosts=${node.total_host_chunks}`);
      }
      if (result.transmission_edges.length > 0) {
        lines.push(``);
        lines.push(`Transmission edges (top 5 by shared_chunks):`);
        const topEdges = result.transmission_edges.slice().sort((a, b) => b.shared_chunks - a.shared_chunks).slice(0, 5);
        for (const e of topEdges) {
          lines.push(`  ${e.from_node_id}  ↔  ${e.to_node_id}   shared=${e.shared_chunks}  bridges=[${e.bridge_witness_ids.slice(0, 3).join(",")}${e.bridge_witness_ids.length > 3 ? ",..." : ""}]`);
        }
      }
      if (result.bridge_witnesses.length > 0) {
        lines.push(``);
        lines.push(`Bridge witnesses (top 5):`);
        for (const b of result.bridge_witnesses.slice(0, 5)) {
          lines.push(`  ${b.tablet_id}  spans=${b.spans_n_nodes} nodes  chunks=[${b.chunks_in_each_node.join(",")}]`);
        }
      }
      if (result.warnings.length > 0) {
        lines.push(``);
      ...
      return structuredResult(lines.join("\n"), {
        schema: SCHEMA,
        data: result,
        provenance: provenance("local", "local:composition-lineage-from-chunk-bfs-and-metadata", VERSION, {
          citation:
            "Transmission lineage: v0.22 BFS witness expansion bucketed by (period × provenance), chunk-sharing edges between nodes, bridge-witness extraction. v0.35.0, §3.22.",
        }),
        warnings: result.warnings.length > 0 ? result.warnings : undefined,
      });
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      return structuredResult(`find_composition_lineage error: ${msg}`, {
        schema: SCHEMA,
        data: {
          query: { composition_id: composition_id ?? null, seed_tablet_id: seed_tablet_id ?? "", max_witnesses: max_witnesses ?? 50 },
          composition: { source: "unresolved", composition_id: null, composition_name: null, inferred_confidence: null },
          witnesses: [] as never[],
          transmission_nodes: [] as never[],
          transmission_edges: [] as never[],
          bridge_witnesses: [] as never[],
          diffusion_summary: {
            n_witnesses: 0, n_distinct_periods: 0, n_distinct_provenances: 0,
            n_nodes: 0, n_edges: 0, n_cross_period_edges: 0,
            n_cross_provenance_edges: 0, n_bridge_witnesses: 0,
...
server.registerTool(
  ...
  {
    description:
      "Probabilistic composition classifier for damaged passages. Accepts a raw signs string (e.g. paste a hand-transliteration) OR a corpus-resident tablet_id; computes joint sign2vec-centroid (v0.23) + canonical-chunk-overlap (v0.20) score against the v0.32 composition registry; emits softmax probability distribution + Shannon entropy. Two innovations over v0.32 identify_composition: (1) raw signs accepted, so non-corpus passages are classifiable; (2) optional restoration-marginalization (marginalize_restorations=true) replaces each X with a weighted mixture of v0.30 lacuna-restorer top-K predictions rather than dropping the position from the centroid. Uncertainty reported via composition_entropy (Shannon bits, 0=certain, log2(n)=uniform). Methods §3.23, claim 43.",
    inputSchema: {
      tablet_id: z.string().optional().describe("Corpus-resident tablet ID. When provided, chunk_overlap axis is enabled."),
      signs: z.string().optional().describe("Raw signs string (space-separated, X/x/? as damage markers). When provided, chunk_overlap axis is disabled. One of {tablet_id, signs} required."),
      top_k: z.number().int().min(1).max(20).optional().describe("Max compositions returned. Default = full registry."),
      marginalize_restorations: z.boolean().optional().describe("If true, each damage position is replaced by a weighted mixture of v0.30 top-K sign restorations in the centroid. Default false (drop damage positions)."),
      restoration_top_k: z.number().int().min(1).max(20).optional().describe("Top-K restorations per damage position when marginalizing. Default 5."),
      temperature: z.number().min(0.01).max(2.0).optional().describe("Softmax temperature. Lower = sharper distribution; higher = flatter. Default 0.1."),
    },
  },
  async ({ tablet_id, signs, top_k, marginalize_restorations, restoration_top_k, temperature }) => {
    const SCHEMA = schemaId("damaged_passage_composition_probability");
    try {
      const result = damagedPassageCompositionProbability({
        tabletId: tablet_id,
        signs,
        topK: top_k,
        marginalizeRestorations: marginalize_restorations,
        restorationTopK: restoration_top_k,
        temperature,
      });
      const q = result.query;
      const u = result.uncertainty;
      const lines: string[] = [
        `Source: ${q.source}${q.tablet_id ? ` (${q.tablet_id})` : ""}  ·  signs=${q.n_signs_total}  ·  visible=${q.n_signs_visible}  ·  damaged=${q.n_signs_damaged}  ·  lacuna=${q.lacuna_density?.toFixed(3) ?? "n/a"}`,
        `Entropy: ${u.composition_entropy.toFixed(3)} bits  ·  max_prob=${u.max_probability.toFixed(3)}  ·  marginalized=${u.restoration_marginalization_applied}${u.restoration_marginalization_applied ? ` (${u.restored_positions_used} positions restored)` : ""}  ·  embedded_signs=${u.n_signs_with_embedding}`,
        ``,
        `Probability distribution:`,
        ``,
      ];
      for (const [i, c] of result.candidates.entries()) {
        lines.push(`${i + 1}. ${c.composition_name} [${c.composition_type}]  ·  p=${c.probability.toFixed(3)}  ·  raw=${c.raw_score.toFixed(3)}`);
        lines.push(`     cosine=${c.axis_scores.sign2vec_centroid.cosine.toFixed(3)}${c.axis_scores.sign2vec_centroid.applicable ? "" : " (n/a)"}  ·  chunks=${c.axis_scores.chunk_overlap.shared_canonical_chunks}/${c.axis_scores.chunk_overlap.canonical_chunks_count}${c.axis_scores.chunk_overlap.applicable ? "" : " (n/a)"}`);
      }
      if (result.warnings.length > 0) {
        lines.push(``);
        lines.push(`Warnings: ${result.warnings.join("; ")}`);
      ...
      return structuredResult(lines.join("\n"), {
        schema: SCHEMA,
        data: result,
        provenance: provenance("local", "local:damaged-passage-composition-probability", VERSION, {
          citation:
            "Probabilistic composition classifier with optional restoration marginalization. Joint v0.23 sign2vec centroid + v0.20 chunk overlap + v0.30 lacuna restorer, softmax-normalized over the v0.32 registry. v0.36.0, §3.23.",
        }),
        warnings: result.warnings.length > 0 ? result.warnings : undefined,
      });
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      return structuredResult(`damaged_passage_composition_probability error: ${msg}`, {
        schema: SCHEMA,
        data: {
          query: { source: signs ? "signs" : "tablet_id", tablet_id: tablet_id ?? null, n_signs_total: 0, n_signs_visible: 0, n_signs_damaged: 0, lacuna_density: null },
          candidates: [] as never[],
          uncertainty: { composition_entropy: 0, restoration_marginalization_applied: false, restored_positions_used: 0, n_signs_with_embedding: 0, max_probability: 0 },
          index_stats: { chunk_index_loaded: false, sign_embeddings_loaded: false, signs_cache_loaded: false },
          warnings: [msg],
        },
        provenance: provenance("local", "local:damaged-passage-composition-probability", VERSION),
        warnings: [msg],
...
// ─── v0.37.0 — list_compositions (registry introspection) ───────────────

server.registerTool(
  "list_compositions",
  {
    description:
      "Return the full v0.32+ composition registry as a structured payload, including registry version, license, persistent URIs, print_editions, external_ids (eBL/OGSL/CAD), and exemplar_tablets per composition. Per panel-review §3.24, the registry is now a separately-citable artifact (data/compositions-v1.json) so two papers referring to 'cuneiform-mcp v0.37 registry v1.0.0' can be guaranteed to share the same registry state. Use this tool for: methodology-paper citations, composition-discovery (when you don't know the composition_id), validation of registry contents.",
    inputSchema: {
      include_tablets: z.boolean().optional().describe("Include exemplar_tablets[] for each composition. Default true."),
      include_print_editions: z.boolean().optional().describe("Include print_editions[] for each composition. Default true."),
    },
  },
  async ({ include_tablets, include_print_editions }) => {
    const SCHEMA = schemaId("list_compositions");
    try {
      const meta = registryMetadata();
      const compositions = listCompositions().map((c) => ({
        id: c.id,
        name: c.name,
        name_akkadian: c.name_akkadian,
        description: c.description,
        composition_type: c.composition_type,
        typical_genre: c.typical_genre,
        typical_period: c.typical_period,
        parent_curriculum: c.parent_curriculum,
        uri: c.uri,
        external_ids: c.external_ids,
        paper_sections: c.paper_sections,
        exemplar_tablets: include_tablets !== false ? c.exemplar_tablets : [],
        print_editions: include_print_editions !== false ? c.print_editions : [],
      }));
      const lines: string[] = [
        `Registry v${meta.registry_version}  ·  ${meta.n_compositions} compositions  ·  ${meta.license}  ·  ${meta.registry_uri}`,
        ``,
      ];
      for (const c of compositions) {
        lines.push(`${c.id} — ${c.name} [${c.composition_type}]  ·  ${c.exemplar_tablets.length} exemplars  ·  ${c.paper_sections.join(", ")}`);
        if (c.print_editions.length > 0) {
          lines.push(`   ${c.print_editions[0].citation}: ${c.print_editions[0].title} (${c.print_editions[0].series})`);
        }
      }
      ...
      return structuredResult(lines.join("\n"), {
        ...
        provenance: provenance("local", "local:composition-registry-v1", VERSION, {
          citation:
            "Versioned composition registry artifact (data/compositions-v1.json, registry v1.0.0). Per panel-review §3.24, registry is separately citable.",
        }),
      });
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      return structuredResult(`list_compositions error: ${msg}`, {
        schema: SCHEMA,
        data: { registry_metadata: null, compositions: [] as never[] },
        provenance: provenance("local", "local:composition-registry-v1", VERSION),
        warnings: [msg],
      });
    }
...
// ─── v0.39.0 — render_stemma_svg (Newick → SVG visualization) ────────────

server.registerTool(
  "render_stemma_svg",
  {
    description:
      "Render a Newick stemma string (typically from v0.33 build_stemma_with_rooting or v0.22 build_canonical_recension_tree) as a self-contained SVG suitable for direct embedding in HTML/Markdown or saving to a .svg file. Cladogram-style layout: horizontal branches, branch-length-proportional x-positions, leaf labels right-aligned. Per panel-review §3.24 / Yamamoto: bridges the gap between the Newick string output of v0.22/v0.33 and the tree-picture format scholars need for dissertation chapters.",
    inputSchema: {
      newick: z.string().min(1).describe("Newick string. Must end with ';'."),
      width: z.number().int().min(200).max(4000).optional().describe("SVG canvas width in px. Default 800."),
      height: z.number().int().min(100).max(8000).optional().describe("SVG canvas height in px. Default auto: 30*leaf_count + 80."),
      margin: z.number().int().min(0).max(200).optional().describe("Margin around tree in px. Default 40."),
      branch_scale: z.number().positive().optional().describe("Pixels per unit branch length. Auto-fit if omitted."),
      font_size: z.number().int().min(8).max(48).optional().describe("Label font size in px. Default 12."),
      show_internal_labels: z.boolean().optional().describe("Render internal-node labels (N1, N2, ...). Default false."),
      title: z.string().optional().describe("Optional title rendered above the tree."),
    },
  },
  async ({ newick, width, height, margin, branch_scale, font_size, show_internal_labels, title }) => {
    const SCHEMA = schemaId("render_stemma_svg");
    try {
      const result = renderStemmaSvg({
        newick,
        width,
        height,
        margin,
        ...
        fontSize: font_size,
        showInternalLabels: show_internal_labels,
        title,
      });
      const lines: string[] = [
        `Stemma SVG: ${result.width}x${result.height}px  ·  ${result.leaf_count} leaves  ·  ${result.internal_count} internal nodes  ·  total_bl=${result.total_branch_length.toFixed(3)}`,
      ];
      if (result.warnings.length > 0) {
        lines.push(``);
        lines.push(`Warnings: ${result.warnings.join("; ")}`);
        ...
        lines.push(``);
        lines.push(`SVG ready (${result.svg.length} chars). First 200 chars:`);
        lines.push(result.svg.slice(0, 200) + (result.svg.length > 200 ? "..." : ""));
      }
      return structuredResult(lines.join("\n"), {
        schema: SCHEMA,
        data: result,
        provenance: provenance("local", "local:newick-to-svg-renderer", VERSION, {
          citation:
            "Cladogram-style Newick renderer for stemma visualization. Self-contained SVG, no external dependencies. v0.39.0, §3.24 panel response.",
        }),
        warnings: result.warnings.length > 0 ? result.warnings : undefined,
      });
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      return structuredResult(`render_stemma_svg error: ${msg}`, {
        schema: SCHEMA,
        data: { svg: "", width: 0, height: 0, leaf_count: 0, internal_count: 0, total_branch_length: 0, warnings: [msg] },
        provenance: provenance("local", "local:newick-to-svg-renderer", VERSION),
        warnings: [msg],
      });
    }
...
// ─── v0.39.0 — get_tablet_image_links (IIIF + provenance) ───────────────
...
server.registerTool(
  ...
  async ({ tablet_id }) => {
    const SCHEMA = schemaId("get_tablet_image_links");
    try {
      const meta = _getFragmentMetadataForImages(tablet_id);
      const ancient = meta ? getAncientFindSpot(meta) : null;
      ...
      return structuredResult(lines.join("\n"), {
        ...
        provenance: provenance("eBL", "ebl.lmu.de/fragmentarium", VERSION, {
          citation:
            "IIIF / eBL imagery links + ancient find-spot. Photos hosted by Munich eBL group. v0.39.0, §3.24 panel response.",
        }),
      });
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
...
// 116. search_sumtablets — LIVE (cached SumTablets metadata search).
// ─── v0.79.0 — search_sumtablets (Sumerian corpus metadata surface) ─────
//
// SumTablets (Simmons, Diehl-Martinez & Jurafsky, ML4AL @ ACL 2024; HF
// colesimmons/SumTablets, CC-BY-4.0) is fetched to cache (NOT committed) and
// ABZ-converted into ~/.cache/cuneiform-mcp/sumtablets-signs.json so its ~91.6K
// Sumerian tablets become first-class chunk-index / find_chunk_parallels hosts.
// Those generic chunk tools host the tablets but do NOT expose SumTablets' rich
// native metadata (period, genre, Latin transliteration). search_sumtablets is
// that metadata surface: filter by id / period / genre / transliteration
// substring. Reads the cached metadata sidecar (sumtablets-meta.json), so
// hyparquet stays a build-time-only dependency. Surfaces the severe corpus skew
// (~86% Ur III / ~94% Administrative) so callers don't over-read repetitive
// admin formulae as philological parallels. Sumerian tablets are NOT eBL
// compositions — no composition-assignment fields are returned.

server.registerTool(
  "search_sumtablets",
  {
    description:
      "Search the SumTablets Sumerian corpus (Simmons, Diehl-Martinez & Jurafsky, ML4AL @ ACL 2024; HF colesimmons/SumTablets, CC-BY-4.0) by id / period / genre / transliteration substring. SumTablets is fetched to your local cache (NOT committed — CC-BY-4.0, attribution required) and ABZ-converted so its ~91.6K Sumerian tablets become first-class chunk-index / find_chunk_parallels hosts; this tool is the complementary metadata surface the generic chunk tools don't expose (CDLI P-number id, period, genre, Latin transliteration). Returns matched records + per-result period/genre distributions, and warns about the corpus's severe skew (~86% Ur III, ~94% Administrative) so shared 'chunks' dominated by repetitive admin formulae aren't mis-read as philological parallels. Sumerian tablets are NOT eBL compositions — this tool returns NO composition assignment (compute_quotation_network resolves them to null and skips them). Requires the cache to be built: node scripts/fetch-sumtablets.mjs && node scripts/build-sumtablets-abz-map.mjs && node scripts/build-sumtablets-signs.mjs — degrades gracefully (available:false + a setup hint) when absent.",
    inputSchema: {
      id: z
        .string()
        .optional()
        .describe("Substring match on the CDLI P-number id, e.g. 'P11247' or 'P112475'."),
      period: z
        .string()
        .optional()
        .describe("Substring match on period, e.g. 'Ur III', 'Old Babylonian', 'ED'."),
      genre: z
        .string()
        .optional()
        .describe("Substring match on genre, e.g. 'Administrative', 'Royal', 'Literary'."),
      transliteration_contains: z
        .string()
        .optional()
        .describe("Case-insensitive substring match against the Latin transliteration."),
      limit: z
        .number()
        .int()
        .min(1)
        .max(200)
        .optional()
        .describe("Max records to return (default 25, cap 200)."),
    },
  },
  async ({ id, period, genre, transliteration_contains, limit }) => {
    const SCHEMA = schemaId("search_sumtablets");
    const r = searchSumtablets({ id, period, genre, transliteration_contains, limit });
    const lines: string[] = [];
    if (!r.available) {
      lines.push("SumTablets metadata cache not found — see warnings for setup steps.");
    } else {
      lines.push(
        `SumTablets: ${r.match_count} match${r.match_count === 1 ? "" : "es"} of ${r.total_corpus} tablets (showing ${r.returned})`,
      );
      const topP = Object.entries(r.period_distribution).slice(0, 4);
      const topG = Object.entries(r.genre_distribution).slice(0, 4);
      if (topP.length) lines.push(`  periods: ${topP.map(([k, v]) => `${k}=${v}`).join(", ")}`);
      if (topG.length) lines.push(`  genres:  ${topG.map(([k, v]) => `${k}=${v}`).join(", ")}`);
      lines.push("");
      for (const rec of r.results) {
        const tl = rec.transliteration.replace(/\s+/g, " ").trim().slice(0, 80);
        lines.push(
          `  ${rec.id} · ${rec.period ?? "?"} · ${rec.genre ?? "?"}${tl ? ` — ${tl}${rec.transliteration.length > 80 ? "…" : ""}` : ""}`,
        );
      }
    }
    return structuredResult(lines.join("\n"), {
      schema: SCHEMA,
      data: r,
      provenance: provenance("CDLI", "local:sumtablets-meta.json", VERSION, {
        citation:
          "SumTablets — Simmons, Diehl-Martinez & Jurafsky, 'SumTablets: A Transliteration Dataset of Sumerian Tablets', ML4AL @ ACL 2024. HF colesimmons/SumTablets. CC-BY-4.0. CDLI-derived P-numbers + transliterations; fetched to local cache only, not redistributed.",
      }),
      warnings: r.warnings.length > 0 ? r.warnings : undefined,
    });
  },
...
// ─── v0.40.0 — compute_confidence_calibration (reliability diagram) ──────

server.registerTool(
  "compute_confidence_calibration",
  {
    description:
      "Compute a reliability-diagram + Brier score + Expected Calibration Error (ECE) + Maximum Calibration Error (MCE) from labeled (predicted_probability, correct) pairs. Per panel-review §3.24 / Lindqvist: when a tool says p=0.989, is the true accuracy at that confidence bin actually ~99%? This tool measures it. Overconfident classifiers have observed_accuracy < mean_predicted_probability per bin; well-calibrated classifiers have ECE ≤ 0.05. Typical workflow: run damaged_passage_composition_probability or identify_composition on a known-composition tablet set, collect (top_candidate.probability, top_candidate.composition_id === ground_truth) pairs, feed to this tool.",
    inputSchema: {
      samples: z.array(z.object({
        predicted_probability: z.number().min(0).max(1),
        correct: z.boolean(),
      })).min(1).describe("Array of (predicted_probability, correct) pairs."),
      n_bins: z.number().int().min(2).max(100).optional().describe("Number of bins for the reliability diagram. Default 10 (deciles)."),
    },
  },
  async ({ samples, n_bins }) => {
    const SCHEMA = schemaId("compute_confidence_calibration");
    try {
      const result = computeConfidenceCalibration({
        samples: samples as CalibrationSample[],
        nBins: n_bins,
      });
      const lines: string[] = [
        `Samples: ${result.n_samples}  ·  bins: ${result.n_bins}  ·  verdict: ${result.calibration_verdict}`,
        `Brier: ${result.brier_score.toFixed(4)}  ·  ECE: ${result.expected_calibration_error.toFixed(4)}  ·  MCE: ${result.max_calibration_error.toFixed(4)}`,
        `Overall accuracy: ${result.overall_accuracy.toFixed(4)}  ·  Overall mean probability: ${result.overall_mean_probability.toFixed(4)}  ·  gap: ${(result.overall_accuracy - result.overall_mean_probability).toFixed(4)}`,
        ``,
        `Reliability table:`,
        `  bin    n  mean_p  obs_acc   gap`,
      ];
      for (const b of result.bins) {
        if (b.n_samples === 0) continue;
        lines.push(
          `  [${b.bin_lo.toFixed(2)}-${b.bin_hi.toFixed(2)})  ${String(b.n_samples).padStart(4)}  ${b.mean_predicted_probability.toFixed(3)}  ${b.observed_accuracy.toFixed(3)}  ${b.gap >= 0 ? "+" : ""}${b.gap.toFixed(3)}`,
        );
      }
      if (result.warnings.length > 0) {
        lines.push(``);
      ...
      return structuredResult(lines.join("\n"), {
        schema: SCHEMA,
        data: result,
        provenance: provenance("local", "local:confidence-calibration", VERSION, {
          citation:
            "Reliability diagram + Brier + ECE/MCE per panel-review §3.24 Lindqvist ask. v0.40.0.",
        }),
        warnings: result.warnings.length > 0 ? result.warnings : undefined,
      });
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      return structuredResult(`compute_confidence_calibration error: ${msg}`, {
        schema: SCHEMA,
        data: {
          n_samples: 0, n_bins: n_bins ?? 10, bins: [] as never[],
          brier_score: 0, expected_calibration_error: 0, max_calibration_error: 0,
          overall_accuracy: 0, overall_mean_probability: 0,
          calibration_verdict: "insufficient_data" as const,
          warnings: [msg],
        },
        provenance: provenance("local", "local:confidence-calibration", VERSION),
        warnings: [msg],
...
// ─── v0.42.0 — find_sign_glyph (ABZ → Unicode cuneiform glyph) ───────────

server.registerTool(
  "find_sign_glyph",
  {
    description:
      "Convert ABZ codes (e.g. ABZ480) to Unicode cuneiform glyphs (e.g. 𒋮). Accepts either a signs string (space-separated tokens; X/x/? treated as damage) or an array of bare ABZ codes. Resolves ABZ-prefixed tokens via a cached OGSL Labasi ∩ eBL /signs join (cache at ~/.cache/cuneiform-mcp/abz-glyph-map.json, built by scripts/build-abz-glyph-map.mjs). Composite-sign tokens like 'ABZ331e+152i' or 'LAGAB×HAL' return null glyph (no canonical Unicode codepoint). Renders a glyph string with bracketed tokens for unresolved entries; damage positions render as '·' (or caller-supplied damage_glyph). Methods §3.26 (Tier-3 #11). Pure data-loading tool — no novel methodology.",
    inputSchema: {
      signs: z.string().optional().describe("Space-separated signs string (e.g. 'ABZ480 ABZ075 X ABZ319'). One of {signs, abz_codes} required."),
      abz_codes: z.array(z.string()).optional().describe("Array of bare ABZ codes (e.g. ['ABZ480','ABZ319']). Alternative to signs."),
      include_damage_placeholder: z.boolean().optional().describe("Include the damage glyph in rendered_glyph_string. Default true."),
      damage_glyph: z.string().optional().describe("Character used for damage positions. Default '·'."),
    },
  },
  async ({ signs, abz_codes, include_damage_placeholder, damage_glyph }) => {
    const SCHEMA = schemaId("find_sign_glyph");
    try {
      const result = findSignGlyph({
        signs,
        abz_codes,
        include_damage_placeholder,
        damage_glyph,
      });
      const lines: string[] = [
        `Tokens: ${result.query.n_tokens}  ·  resolved=${result.query.n_resolved}  ·  damage=${result.query.n_damage}  ·  unresolved=${result.query.n_unresolved}`,
        `Cache: ${result.glyph_map_stats.cache_loaded ? `v${result.glyph_map_stats.cache_version} (built ${result.glyph_map_stats.cache_built_at})  ·  ${result.glyph_map_stats.n_entries_in_cache} entries` : "NOT LOADED — run scripts/build-abz-glyph-map.mjs"}`,
        ``,
        `Rendered:`,
        result.rendered_glyph_string || "(empty)",
      ];
      if (result.warnings.length > 0) {
        lines.push(``);
        lines.push(`Warnings: ${result.warnings.join("; ")}`);
      ...
      return structuredResult(lines.join("\n"), {
        schema: SCHEMA,
        data: result,
        provenance: provenance("local", "local:abz-glyph-map-v1 (OGSL ∩ eBL)", VERSION, {
          citation:
            "ABZ → Unicode cuneiform glyph lookup. Cache built from OGSL labasi-signs ∩ eBL /signs/{NAME}. v0.42.0, §3.26.",
        }),
        warnings: result.warnings.length > 0 ? result.warnings : undefined,
      });
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      return structuredResult(`find_sign_glyph error: ${msg}`, {
        schema: SCHEMA,
        data: {
          query: { n_tokens: 0, n_resolved: 0, n_damage: 0, n_unresolved: 0 },
          tokens: [] as never[],
          rendered_glyph_string: "",
          glyph_map_stats: { cache_loaded: false, cache_version: null, cache_built_at: null, n_entries_in_cache: 0 },
          warnings: [msg],
        },
        provenance: provenance("local", "local:abz-glyph-map-v1 (OGSL ∩ eBL)", VERSION),
        warnings: [msg],
...
// ─── v0.43.0 — extract_citation_network (Tier-3 #10) ──────────────────────

server.registerTool(
  "extract_citation_network",
  {
    description:
      "Mine the scholarly-citation network from data/biblicalParallels.json + data/mesopotamianParallels.json. Builds nodes (scholars parsed from scholarly_attribution[]; parallels) and co-citation edges (two scholars cited in same parallel). Returns ranked scholars by parallels-supported, top co-citation pairs, and 'bridge scholars' (those reaching ≥ min_bridge_reach parallels — these are the load-bearing authorities in the field). Optional filters: filter_to_scholar (subgraph centered on one scholar), filter_to_parallel (the scholars supporting one parallel). Methods §3.27 (Tier-3 #10). Useful for auto-citing methods-paper findings, bibliography auto-completion, and discovering which parallels share scholarly support.",
    inputSchema: {
      top_k: z.number().int().min(1).max(100).optional().describe("Top-K rankings for scholars + co-citation pairs. Default 20."),
      min_bridge_reach: z.number().int().min(2).optional().describe("Minimum parallels-supported count to qualify as 'bridge scholar'. Default 3."),
      filter_to_scholar: z.string().optional().describe("If set, returns subgraph centered on this author_year (e.g. 'Lambert 1985')."),
      filter_to_parallel: z.string().optional().describe("If set, returns scholars + co-citations for one parallel (e.g. 'meso_001')."),
    },
  },
  async ({ top_k, min_bridge_reach, filter_to_scholar, filter_to_parallel }) => {
    const SCHEMA = schemaId("extract_citation_network");
    try {
      const result = extractCitationNetwork({
        topK: top_k,
        minBridgeReach: min_bridge_reach,
        filterToScholar: filter_to_scholar,
        filterToParallel: filter_to_parallel,
      });
      const s = result.network_stats;
      const lines: string[] = [
        `Network: ${s.n_scholars} scholars  ·  ${s.n_parallels} parallels  ·  ${s.n_co_citation_edges} co-citation edges  ·  ${s.n_bridge_scholars} bridge scholars`,
        `Max scholars/parallel: ${s.max_scholars_per_parallel}  ·  max parallels/scholar: ${s.max_parallels_per_scholar}`,
        ``,
        `Top scholars by parallels-supported:`,
      ];
      for (const sc of result.top_scholars_by_parallels.slice(0, 10)) {
        lines.push(`  ${sc.author_year}  ·  ${sc.n_parallels} parallels`);
      }
      lines.push(``);
      lines.push(`Top co-citation pairs:`);
      for (const e of result.top_co_citation_pairs.slice(0, 10)) {
        lines.push(`  ${e.scholar_a}  ↔  ${e.scholar_b}  ·  weight=${e.weight}  ·  parallels=[${e.shared_parallels.slice(0, 3).join(",")}${e.shared_parallels.length > 3 ? ",..." : ""}]`);
      }
      if (result.bridge_scholars.length > 0) {
        lines.push(``);
        lines.push(`Bridge scholars (reach ≥ ${min_bridge_reach ?? 3}):`);
        for (const sc of result.bridge_scholars.slice(0, 5)) {
          lines.push(`  ${sc.author_year}  ·  ${sc.n_parallels} parallels  ·  [${sc.parallels_supported.slice(0, 3).join(",")}${sc.parallels_supported.length > 3 ? ",..." : ""}]`);
        }
      }
      if (result.warnings.length > 0) {
        lines.push(``);
      ...
      return structuredResult(lines.join("\n"), {
        schema: SCHEMA,
        data: result,
        provenance: provenance("local", "local:citation-network-from-parallels", VERSION, {
          citation:
            "Scholarly citation network mined from data/biblicalParallels.json + data/mesopotamianParallels.json scholarly_attribution[]. v0.43.0, §3.27.",
        }),
        warnings: result.warnings.length > 0 ? result.warnings : undefined,
      });
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      return structuredResult(`extract_citation_network error: ${msg}`, {
        schema: SCHEMA,
        data: {
          scholars: [] as never[],
          parallels: [] as never[],
          co_citation_edges: [] as never[],
          top_scholars_by_parallels: [] as never[],
          top_co_citation_pairs: [] as never[],
          bridge_scholars: [] as never[],
          network_stats: { n_scholars: 0, n_parallels: 0, n_co_citation_edges: 0, n_bridge_scholars: 0, max_scholars_per_parallel: 0, max_parallels_per_scholar: 0 },
          warnings: [msg],
        },
        provenance: provenance("local", "local:citation-network-from-parallels", VERSION),
        warnings: [msg],
...
server.registerTool(
  "compute_quotation_network",
  {
    ...
    inputSchema: {
      ...
      min_citations: z
        ...
        .optional()
        .describe(
          "Minimum shared_chunks_count for citation-stream edges. Default 2. Set to 1 to admit every commentary/base pair candidate.",
        ),
      format: z
        .enum(["json", "dot", "summary"])
        .optional()
        .describe("Surface format for the human-readable text payload. Default 'summary'. All three files are written regardless."),
      min_resolution_confidence: z
        .number()
        .min(0)
        .max(1)
        .optional()
        .describe(
          "Floor on the v0.54 composition-assignments cache confidence below which a tablet is dropped from edge participation. Default 0.5. Lower to ~0.35 to surface compositions whose cache rows cluster around the v0.54 axis-uncertainty floor (Maqlû ~0.395, Šumma izbu ~0.377, Šumma ālu ~0.394) — those compositions otherwise never participate because only their 2-4 registry-exemplar tablets resolve, and those exemplars may not appear in the chunk-index.",
        ),
    },
  },
  async ({ min_chunk_length, min_citations, format, min_resolution_confidence, min_edge_weight }) => {
    ...
    try {
      ...
      const lines: string[] = [];
      if (fmt === "json") {
        lines.push(JSON.stringify({ nodes: result.nodes, edges: result.edges, metrics: result.metrics }, null, 2));
      } else if (fmt === "dot") {
        // Re-read the DOT artifact so the text surface matches the artifact on disk.
        try {
          const { readFileSync } = await import("node:fs");
          lines.push(readFileSync(result.output_paths.dot, "utf-8"));
        } catch {
          lines.push(`(unable to read DOT at ${result.output_paths.dot})`);
        }
      } else {
        lines.push(
          `Quotation network: ${result.metrics.total_nodes} compositions  ·  ${result.metrics.total_edges} edges  ·  ${result.metrics.scc_count} SCCs (largest ${result.metrics.scc_largest_size})`,
        );
        lines.push(``);
        lines.push(`Top quoted-from (likely canonical base texts):`);
        for (const q of result.metrics.top_quoted_from) {
          lines.push(`  ${q.composition}  ·  in_degree=${q.in_degree}`);
        }
        lines.push(``);
        lines.push(`Top quoters (likely commentaries / dependents):`);
        for (const q of result.metrics.top_quoters) {
          lines.push(`  ${q.composition}  ·  out_degree=${q.out_degree}`);
        }
        if (result.metrics.isolate_compositions.length > 0) {
          lines.push(``);
          lines.push(`Isolate compositions: ${result.metrics.isolate_compositions.join(", ")}`);
        }
        lines.push(``);
        lines.push(`Edges (top 15 by weight):`);
        for (const e of result.edges.slice(0, 15)) {
          const tabs = e.supporting_tablets.slice(0, 3).join(",");
          lines.push(`  ${e.source_composition} → ${e.target_composition}  ·  ${e.evidence_type}  ·  w=${e.weight.toFixed(2)}  ·  [${tabs}]`);
        }
        lines.push(``);
        lines.push(`Artifacts:`);
        lines.push(`  json:    ${result.output_paths.json}`);
        lines.push(`  dot:     ${result.output_paths.dot}`);
        lines.push(`  summary: ${result.output_paths.summary_md}`);
      }
      if (result.warnings.length > 0) {
        lines.push(``);
        lines.push(`Warnings:`);
      ...
      return structuredResult(lines.join("\n"), {
        schema: SCHEMA,
        data: result,
        provenance: provenance("local", "local:quotation-network-multistream", VERSION, {
          citation:
            "Corpus-wide composition-level directed quotation multigraph aggregating buildCitationGraph (v0.20) and chunkIndex (v0.20) evidence streams; tablet→composition resolution via composition-assignments cache (v0.54) + identifyComposition fallback (v0.32). v0.68.0.",
        }),
        warnings: result.warnings.length > 0 ? result.warnings : undefined,
      });
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      return structuredResult(`compute_quotation_network error: ${msg}`, {
        schema: SCHEMA,
        data: {
          nodes: [] as never[],
          edges: [] as never[],
          metrics: {
            total_nodes: 0,
            total_edges: 0,
            top_quoted_from: [] as never[],
            top_quoters: [] as never[],
            isolate_compositions: [] as never[],
            scc_count: 0,
            scc_largest_size: 0,
          },
          output_paths: { json: "", dot: "", summary_md: "" },
          warnings: [msg],
        },
        provenance: provenance("local", "local:quotation-network-multistream", VERSION),
        warnings: [msg],
...
// ─── v0.69.0 — discover_compositions (unsupervised cluster discovery) ────

server.registerTool(
  "discover_compositions",
  {
    description:
      "Unsupervised cluster discovery over the v0.15 Random-Indexing tablet embeddings (~28K tablets × 300-dim, unit-normalized). Deliberately avoids using any genre labels or exemplar registries as a prior — every OTHER composition-surfacing tool in cuneiform-mcp uses the registry as a prior, this one asks 'what does the embedding space produce unaided?'. Three algorithms: kmeans (k-means++ init + Lloyd's; cheap), hierarchical_ward (default — Ward linkage on centroids with Lance-Williams O(n²) memory, capped at max_tablets=5000), dbscan_like (density-based; exposes outlier-shape clusters). Compares each emergent cluster's centroid against centroids built from the 11-composition registry's exemplar_tablets. Clusters above novelty_threshold (default 0.5 = cosine distance > 0.5 from any registered composition) are surfaced as candidate new compositions, tagged with dominant period / find-spot / genre metadata as a suggested human-readable label. Recovery sanity check: how many registered compositions cleared cosine ≥ 0.5 against some emergent cluster — if 0, the partition is broken; if most, the novelty surface is trustworthy. Writes JSON dump + markdown summary to ~/.cache/cuneiform-mcp/composition-discovery/<iso-ts>/. Honest reporting: either outcome (real candidates surfaced OR no novel structure) is publishable. v0.69.0.",
    inputSchema: {
      min_cluster_size: z
        .number()
        .int()
        ...
        .max(1000)
        .optional()
        .describe("Drop clusters smaller than this before novelty scoring. Default 5. A 2-tablet cluster is noise, not a candidate composition."),
      novelty_threshold: z
        .number()
        .min(0)
        .max(1)
        .optional()
        .describe("Clusters with novelty_score (= 1 − max cosine to any registered centroid) above this threshold are returned as candidates. Default 0.5."),
      algorithm: z
        .enum(["kmeans", "hierarchical_ward", "dbscan_like"])
        .optional()
        .describe("Clustering algorithm. Default 'hierarchical_ward'."),
      k: z
        .number()
        .int()
        .min(2)
        .max(500)
        .optional()
        .describe("Number of clusters for kmeans / cut-point for hierarchical_ward. Default 50. Ignored by dbscan_like."),
      max_tablets: z
        .number()
        .int()
        .min(50)
        .max(28000)
        .optional()
        .describe("Random-subsample cap. Default 5000. hierarchical_ward caps at 5000 (O(n²) memory), dbscan_like caps at 3000."),
    },
  },
  async ({ min_cluster_size, novelty_threshold, algorithm, k, max_tablets }) => {
    const SCHEMA = schemaId("discover_compositions");
    try {
      const result = discoverCompositions({
        min_cluster_size,
        novelty_threshold,
        algorithm,
        k,
      ...
      const lines: string[] = [];
      lines.push(
        `discover_compositions — algorithm=${result.algorithm_used}  ·  clusters=${result.clusters_found}  ·  silhouette=${result.metrics.silhouette_score.toFixed(4)}`,
      );
      lines.push(
        `  subsample=${result.metrics.total_tablets_clustered}  ·  registered_recovered=${result.registered_compositions_recovered}/${result.registered_recovery_detail.length}  ·  candidates=${result.candidate_new_compositions.length}`,
      );
      if (result.candidate_new_compositions.length > 0) {
        lines.push("");
        lines.push(`Top-5 candidate new compositions (sorted by novelty):`);
        for (const [i, c] of result.candidate_new_compositions.slice(0, 5).entries()) {
          const reps = c.representative_tablets.slice(0, 5).join(", ");
          lines.push(
            `  ${(i + 1).toString().padStart(2)}. ${c.cluster_id}  n=${c.tablet_count}  novelty=${c.novelty_score.toFixed(3)}  nearest=${c.nearest_registered_composition.id}(d=${c.nearest_registered_composition.cosine_distance.toFixed(3)})  ${c.suggested_label}`,
          );
        ...
        lines.push("");
        lines.push(
          `No clusters cleared novelty_threshold=${result.parameters.novelty_threshold}. This is a publishable null — the unsupervised partition does not surface structure beyond the existing registry at these parameters.`,
        );
      }
      lines.push("");
      lines.push(`Output: ${result.output_paths.summary_md}`);
      if (result.warnings.length > 0) {
        lines.push("");
        lines.push("Warnings:");
        for (const w of result.warnings) lines.push(`  - ${w}`);
      ...
      return structuredResult(lines.join("\n"), {
        ...
        provenance: provenance(
          "local",
          "local:discover-compositions-unsupervised",
          VERSION,
          {
            citation:
              "Unsupervised clustering over v0.15 Random-Indexing tablet embeddings (Sahlgren 2005). k-means / Ward / DBSCAN-like; novelty scored against v0.37 composition registry centroids. v0.69.0.",
          },
        ),
        warnings: result.warnings.length > 0 ? result.warnings : undefined,
      });
      ...
      const msg = e instanceof Error ? e.message : String(e);
      return structuredResult(`discover_compositions error: ${msg}`, {
        schema: SCHEMA,
        data: {
          algorithm_used: algorithm ?? "hierarchical_ward",
          clusters_found: 0,
          registered_compositions_recovered: 0,
          candidate_new_compositions: [] as never[],
          registered_recovery_detail: [] as never[],
          metrics: {
            silhouette_score: 0,
            total_tablets_clustered: 0,
            tablets_in_candidate_new_compositions: 0,
            embedding_dim: 0,
            iterations: 0,
            converged: false,
          },
          parameters: {
            ...
            novelty_threshold: novelty_threshold ?? 0.5,
            algorithm: algorithm ?? "hierarchical_ward",
            k: k ?? 50,
            max_tablets: max_tablets ?? 5000,
            seed: 20260528,
          },
          output_paths: { json: "", summary_md: "" },
          warnings: [msg],
        },
        provenance: provenance("local", "local:discover-compositions-error", VERSION),
        warnings: [msg],
      });
    }
  },
...
// ─── v0.48.0 — find_provenance_clusters (panel-ask Patel + §3.29) ────────

server.registerTool(
  "find_provenance_clusters",
  {
    description:
      "Cluster tablets by ANCIENT find-spot rather than modern museum collection. With v0.45's collection-fallback, getAncientFindSpot returns a populated string for every metadata-present tablet (Kuyunjik / British Museum / Sippar / Nineveh / ...). This tool groups them and reports per-cluster (a) tablet count, (b) period distribution, (c) collection-prefix distribution, (d) spans_multiple_prefixes flag — empirically exposing find-spots whose tablets are scattered across multiple modern museum prefixes (e.g. Kuyunjik covers BOTH K.* and Sm.* and partially BM.*). Methods §3.29, claim 49. Cross-reference: §3.22 found Mīs pî is single-provenance Kuyunjik-anchored; this tool surfaces the same finding from the corpus-wide direction. Use site_filter to restrict to one cluster, min_tablets to drop sparsely-attested clusters.",
    inputSchema: {
      site_filter: z.string().optional().describe("Restrict to one find-spot string (e.g. 'Kuyunjik'). Default: all clusters."),
      min_tablets: z.number().int().min(1).optional().describe("Minimum tablet count for a cluster to be returned. Default 5."),
      include_null_provenance: z.boolean().optional().describe("Include tablets with no resolvable find-spot in an '(unknown)' cluster. Default false."),
      top_k_per_cluster: z.number().int().min(1).max(200).optional().describe("Max tablets per cluster's top_tablets[] sample. Default 20."),
      top_k_clusters: z.number().int().min(1).max(500).optional().describe("Max clusters in output, sorted by size desc. Default 50."),
    },
  },
  async ({ site_filter, min_tablets, include_null_provenance, top_k_per_cluster, top_k_clusters }) => {
    const SCHEMA = schemaId("find_provenance_clusters");
    try {
      const result = findProvenanceClusters({
        siteFilter: site_filter,
        minTablets: min_tablets,
        includeNullProvenance: include_null_provenance,
        topKPerCluster: top_k_per_cluster,
        topKClusters: top_k_clusters,
      });
      const lines: string[] = [
        `Tablets total: ${result.n_tablets_total}  ·  with provenance: ${result.n_tablets_with_provenance}  ·  null: ${result.n_tablets_null_provenance}`,
        `Clusters (≥${min_tablets ?? 5} tablets): ${result.n_clusters}`,
        ``,
      ];
      for (const c of result.clusters.slice(0, 10)) {
        const periods = c.period_distribution.slice(0, 3).map((p) => `${p.period}:${p.count}`).join(", ");
        const prefixes = c.collection_prefix_distribution.slice(0, 4).map((p) => `${p.prefix}:${p.count}`).join(", ");
        lines.push(`  ${c.cluster_id.padEnd(30)} n=${String(c.n_tablets).padStart(5)} periods=[${periods}] prefixes=[${prefixes}]${c.spans_multiple_prefixes ? " *MULTI*" : ""}`);
      }
      if (result.warnings.length > 0) {
        lines.push(``);
        lines.push(`Warnings: ${result.warnings.slice(0, 2).join("; ")}`);
      }
      return structuredResult(lines.join("\n"), {
        schema: SCHEMA,
        data: result,
        provenance: provenance("local", "local:provenance-clusters-from-metadata", VERSION, {
          citation:
            "Ancient find-spot clustering via getAncientFindSpot (v0.45 collection-fallback). Methods §3.29, panel-ask Patel.",
        }),
        warnings: result.warnings.length > 0 ? result.warnings : undefined,
      });
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      return structuredResult(`find_provenance_clusters error: ${msg}`, {
        schema: SCHEMA,
        data: {
          query: { site_filter: site_filter ?? null, min_tablets: min_tablets ?? 5, include_null_provenance: include_null_provenance ?? false, top_k_per_cluster: top_k_per_cluster ?? 20 },
          clusters: [] as never[],
          n_clusters: 0, n_tablets_total: 0, n_tablets_with_provenance: 0, n_tablets_null_provenance: 0,
          warnings: [msg],
        },
        provenance: provenance("local", "local:provenance-clusters-from-metadata", VERSION),
        warnings: [msg],
...
server.registerTool(
  ...
  {
    description:
      "Cross-axis composition-classification audit. Runs identify_composition (sign-trigram + sign2vec centroid + chunk-overlap) AND find_lemma_parallel (lemma-Jaccard, second-hop composition inference via the top neighbor) on the same tablet and reports agreement. Output classes: 'agree' = both axes return the same composition; 'lemma_silent' = query has zero lemmas in cache (eBL gap, e.g. K.5896); 'disagree' = axes return different compositions (methodology signal); 'both_silent' = neither axis can classify. Methods §3.30, Claim 50.",
    inputSchema: {
      tablet_id: z.string().min(1).describe("Query tablet museum number."),
      top_k: z.number().int().min(1).max(20).optional().describe("Top-K results from each axis. Default 5."),
    },
  },
  async ({ tablet_id, top_k }) => {
    const SCHEMA = schemaId("compute_axis_disagreement");
    try {
      const result = computeAxisDisagreement({ tabletId: tablet_id, topK: top_k });
      const lines: string[] = [
        `Query: ${result.query.tablet_id}  ·  agreement: ${result.agreement.toUpperCase()}`,
        ``,
        `Composition axis:`,
        `  top: ${result.composition_axis.top_composition_id ?? "(none)"} ` +
          (result.composition_axis.confidence !== null
            ? `(conf=${result.composition_axis.confidence.toFixed(3)})`
            : ""),
        ``,
        `Lemma axis:`,
        `  n_lemmas=${result.lemma_axis.n_lemmas}  cache_loaded=${result.lemma_axis.cache_loaded}`,
        `  inferred composition: ${result.lemma_axis.inferred_composition_id ?? "(none)"}` +
          (result.lemma_axis.inferred_via_neighbor
            ? ` (via ${result.lemma_axis.inferred_via_neighbor})`
            : ""),
        ``,
      ...
      return structuredResult(lines.join("\n"), {
        schema: SCHEMA,
        data: result,
        provenance: provenance("local", "local:axis-disagreement-id-vs-lemma", VERSION, {
          citation:
            "Cross-axis disagreement audit per §3.30 (Claim 50). v0.49.0.",
        }),
        warnings: result.warnings.length > 0 ? result.warnings : undefined,
      });
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      return structuredResult(`compute_axis_disagreement error: ${msg}`, {
        schema: SCHEMA,
        data: {
          query: { tablet_id, top_k: top_k ?? 5 },
          composition_axis: { top_composition_id: null, top_composition_name: null, confidence: null, candidates: [] as never[] },
          lemma_axis: { n_lemmas: 0, cache_loaded: false, top_neighbors: [] as never[], inferred_composition_id: null, inferred_composition_name: null, inferred_via_neighbor: null },
          agreement: "both_silent" as const,
          rationale: msg,
          warnings: [msg],
        },
        provenance: provenance("local", "local:axis-disagreement-id-vs-lemma", VERSION),
...
// ─── v0.50.0 — recalibrate_lacuna_scores (Platt scaling) ─────────────────

server.registerTool(
  "recalibrate_lacuna_scores",
  {
    description:
      "Fit Platt-scaling logistic regression on ~/.cache/cuneiform-mcp/lacuna-bleu-calibration-samples.json (produced by scripts/benchmark-lacuna-bleu.mjs) and report ECE before/after recalibration. Closes the §3.25 overconfidence finding (joint_score is a ranking signal, not P(correct)): the v0.40 benchmark exposed ECE 0.6490 at 50 samples; with 500 samples + Platt scaling, post-calibration ECE should drop closer to the well-calibrated threshold (0.05). Outputs {platt_a, platt_b} parameters; consumers can call applyPlattCalibration(raw, params) to recalibrate any joint_score. Methods §3.31, Claim 51.",
    inputSchema: {
      method: z.enum(["platt"]).optional().describe("Calibration method. Currently only 'platt' (logistic regression on logit-transformed inputs). Default platt."),
      n_bins: z.number().int().min(2).max(50).optional().describe("Bins for the reliability diagram. Default 10."),
    },
  },
  async ({ method, n_bins }) => {
    const SCHEMA = schemaId("recalibrate_lacuna_scores");
    try {
      const result = recalibrateLacunaScores({ method, nBins: n_bins });
      const lines: string[] = [
        `Source: ${result.source_file}  ·  n_samples: ${result.n_samples}`,
        ``,
        `Before calibration:`,
        `  ECE=${result.before.expected_calibration_error.toFixed(4)}  Brier=${result.before.brier_score.toFixed(4)}  verdict=${result.before.verdict}`,
        `  observed_acc=${result.before.observed_accuracy.toFixed(4)}  mean_predicted=${result.before.mean_predicted.toFixed(4)}`,
        ``,
        `After Platt (a=${result.after.platt_a.toFixed(3)}, b=${result.after.platt_b.toFixed(3)}):`,
        `  ECE=${result.after.expected_calibration_error.toFixed(4)}  Brier=${result.after.brier_score.toFixed(4)}  verdict=${result.after.verdict}`,
        `  observed_acc=${result.after.observed_accuracy.toFixed(4)}  mean_predicted=${result.after.mean_predicted.toFixed(4)}`,
        ``,
        `ECE reduction: ${result.improvement.ece_reduction.toFixed(4)}  (${isFinite(result.improvement.ece_reduction_factor) ? result.improvement.ece_reduction_factor.toFixed(1) + "×" : "∞"} improvement)`,
        ``,
      ...
      return structuredResult(lines.join("\n"), {
        schema: SCHEMA,
        data: result,
        provenance: provenance("local", "local:platt-calibration-on-lacuna-bleu-samples", VERSION, {
          citation:
            "Platt-scaling logistic-regression recalibration of restore_lacuna_semantic joint_score. v0.50.0, §3.31.",
        }),
        warnings: result.warnings.length > 0 ? result.warnings : undefined,
      });
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      return structuredResult(`recalibrate_lacuna_scores error: ${msg}`, {
        schema: SCHEMA,
        data: {
          source_file: "", n_samples: 0,
...
// ─── v0.52.0 — recommend_validation_target (active-learning prioritizer) ─

server.registerTool(
  "recommend_validation_target",
  {
    description:
      "Active-learning prioritizer: closes the v0.31 validation-resolutions loop. Builds a candidate pool from chunk-index co-host pairs, ranks by chunk-overlap percentile, returns pairs in the MID-band (~50th percentile) as the highest-information-gain labeling targets. Theory: pairs with very high chunk-overlap are confidently-positive (low info gain); very low overlap are confidently-negative (low gain); mid-overlap is where the model is uncertain (high gain). Filters out pairs already in the validation-resolutions store. Output reports v1_progress: n_positives_in_store vs the v1.0 ≥100-positives target (panel-ask G1). The CHEAP version — uses chunk-overlap as a proxy for v0.29 model uncertainty rather than running full extractPairFeatures (~30s/pair) on every candidate; the Bayesian-uncertainty version is deferred to v0.53+ pending feature-cache infrastructure. Methods §3.33, Claim 53.",
    inputSchema: {
      top_k: z.number().int().min(1).max(50).optional().describe("Number of candidate pairs to return. Default 10."),
      n_anchor_tablets: z.number().int().min(1).max(500).optional().describe("Top-N chunk-host tablets to use as pair anchors. Default 100."),
      pairs_per_anchor: z.number().int().min(1).max(50).optional().describe("Co-host neighbors to consider per anchor. Default 10."),
      exclude_already_resolved: z.boolean().optional().describe("Skip pairs already in validation-resolutions store. Default true."),
    },
  },
  async ({ top_k, n_anchor_tablets, pairs_per_anchor, exclude_already_resolved }) => {
    const SCHEMA = schemaId("recommend_validation_target");
    try {
      const result = recommendValidationTarget({
        topK: top_k,
        nAnchorTablets: n_anchor_tablets,
        pairsPerAnchor: pairs_per_anchor,
        excludeAlreadyResolved: exclude_already_resolved,
      });
      const v1 = result.v1_progress;
      const ps = result.pool_stats;
      const lines: string[] = [
        `v1.0 progress: ${v1.n_positives_in_store + 12}/${v1.v1_target} positives in store (${v1.pairs_needed_to_reach_target} more needed)`,
        `Pool: considered ${ps.candidate_pairs_considered} pairs · resolved skip ${ps.already_resolved_skipped} · in-pool ${ps.in_pool_after_filter}`,
        `Chunk-overlap range: min=${ps.overlap_min} median=${ps.overlap_median} max=${ps.overlap_max}`,
        ``,
        `Top ${result.candidates.length} candidates (sorted by uncertainty_proxy desc):`,
        ``,
      ];
      for (const [i, c] of result.candidates.entries()) {
        lines.push(
          `${i + 1}. ${c.tablet_a.padEnd(15)} ↔ ${c.tablet_b.padEnd(15)}  chunks=${c.chunk_overlap}  pctile=${(c.overlap_percentile * 100).toFixed(0)}%  uncertainty=${c.uncertainty_proxy.toFixed(3)}`,
        );
        lines.push(`     ${c.rationale}`);
      }
      if (result.warnings.length > 0) {
      ...
      return structuredResult(lines.join("\n"), {
        schema: SCHEMA,
        data: result,
        provenance: provenance("local", "local:active-learning-prioritizer", VERSION, {
          citation:
            "Information-gain-prioritized validation target picker. Selects pairs near the v0.29 decision boundary for maximum labeling value. v0.52.0, §3.33.",
        }),
        warnings: result.warnings.length > 0 ? result.warnings : undefined,
      });
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      return structuredResult(`recommend_validation_target error: ${msg}`, {
        schema: SCHEMA,
        data: {
          query: { top_k: top_k ?? 10, n_anchor_tablets: n_anchor_tablets ?? 100, pairs_per_anchor: pairs_per_anchor ?? 10, exclude_already_resolved: exclude_already_resolved ?? true },
          candidates: [] as never[],
          pool_stats: { candidate_pairs_considered: 0, already_resolved_skipped: 0, in_pool_after_filter: 0, overlap_min: 0, overlap_median: 0, overlap_max: 0 },
          v1_progress: { n_positives_in_store: 0, v1_target: 100, pairs_needed_to_reach_target: 100 },
          warnings: [msg],
        },
        provenance: provenance("local", "local:active-learning-prioritizer", VERSION),
        warnings: [msg],
...
// ─── v0.55.0 — list_candidate_exemplars (discovery → review handoff) ─────

server.registerTool(
  "list_candidate_exemplars",
  {
    description:
      "Surface discovered candidate exemplars from the v0.54 composition-assignments cache (~/.cache/cuneiform-mcp/composition-assignments.json, built by scripts/build-corpus-composition-assignments.mjs). Each candidate is a tablet that v0.32 identify_composition classified at p ≥ min_confidence to a registry composition but is NOT in that composition's exemplar_tablets list. Each carries a suggested pair (candidate + composition's first registered exemplar) ready for record_validation_resolution. Closes the discovery → scholarly-review handoff for the v1.0 G1 ≥100-positives gate. Filter by composition_id to focus on one composition's expansion candidates. Methods §3.35, Claim 55.",
    inputSchema: {
      composition_id: z.string().optional().describe("Filter to one composition's candidates (e.g. 'mis_pi'). Default: all compositions."),
      min_confidence: z.number().min(0).max(1).optional().describe("Minimum v0.32 classification confidence. Default 0.9."),
      exclude_already_in_store: z.boolean().optional().describe("Skip candidates whose suggested_pair_id is already in validation-resolutions store. Default true."),
      top_k: z.number().int().min(1).max(500).optional().describe("Max candidates returned. Default 50."),
    },
  },
  async ({ composition_id, min_confidence, exclude_already_in_store, top_k }) => {
    const SCHEMA = schemaId("list_candidate_exemplars");
    try {
      const result = listCandidateExemplars({
        compositionId: composition_id,
        minConfidence: min_confidence,
        excludeAlreadyInStore: exclude_already_in_store,
        topK: top_k,
      });
      const lines: string[] = [
        `Cache: ${result.cache_stats.cache_loaded ? `${result.cache_stats.total_assignments} assignments (built ${result.cache_stats.cache_built_at})` : "NOT LOADED — run scripts/build-corpus-composition-assignments.mjs"}`,
        `Store: ${result.store_stats.n_positive_in_store} positives + ${result.store_stats.n_uncertain_in_store} uncertain · v1.0 target: ${result.store_stats.v1_target}`,
        ``,
        `Totals by composition (all candidates at p ≥ ${(min_confidence ?? 0.9).toFixed(2)}):`,
      ];
      for (const t of result.totals_by_composition) {
        lines.push(`  ${t.composition_id.padEnd(20)} ${t.count}`);
      }
      lines.push(``);
      lines.push(`Top ${result.candidates.length} candidates (sorted by confidence desc):`);
      for (const c of result.candidates.slice(0, 15)) {
        lines.push(`  ${c.tablet_id.padEnd(15)} → ${c.composition_id.padEnd(15)} p=${c.confidence.toFixed(3)}  pair_anchor=${c.suggested_pair_anchor}  genre=${(c.primary_genre || "?").slice(0, 30)}`);
      }
      if (result.warnings.length > 0) {
        lines.push(``);
        lines.push(`Warnings: ${result.warnings[0]}`);
      ...
      return structuredResult(lines.join("\n"), {
        schema: SCHEMA,
        data: result,
        provenance: provenance("local", "local:discovered-candidate-exemplars", VERSION, {
          citation:
            "v0.54 corpus composition-assignment cache → review-ready candidates. Discovery → review handoff. v0.55.0, §3.35.",
        }),
        warnings: result.warnings.length > 0 ? result.warnings : undefined,
      });
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      return structuredResult(`list_candidate_exemplars error: ${msg}`, {
        schema: SCHEMA,
        data: {
          query: { composition_id_filter: composition_id ?? null, min_confidence: min_confidence ?? 0.9, exclude_already_in_store: exclude_already_in_store ?? true, top_k: top_k ?? 50 },
          candidates: [] as never[],
          totals_by_composition: [] as never[],
...
server.registerTool(
  ...
  {
    description:
      "Genre-Conflict Sentinel. Surfaces tablets where identify_composition's high-confidence composition-FAMILY (magic / divination / lexical, from registry typical_genre) disagrees with the tablet's eBL editorial genre-FAMILY (medicine / magic / divination / literature / lexical, from primary_genre), and characterizes the shared-text evidence by the RARITY of the length-20 sign-trigram window linking the tablet to a composition exemplar. Each hit is CLASSIFIED: 'formulaic' (the shared window is a pan-corpus boilerplate hub — weak, the majority case), 'likely_misassignment' (a rare shared passage that DOMINATES the tablet → the model is probably just wrong about its genre), or 'embedded_quotation_candidate' (a rare shared passage LOCALIZED inside an otherwise-on-genre tablet — the genuine 'incantation embedded in a medical text' phenomenon, e.g. K.2433 magic-in-medicine, overlap_fraction 0.12). HONESTY: the shared window is 20 sign-trigrams (may span breaks), NOT 20 contiguous verbatim signs; and at the default min_shared_chunks=1 corroboration removes ~0 cross-family hits because identify_composition is itself chunk-weighted on these exemplars — so corroboration is NOT an independent check, only rarest_window_host_count carries information. INTEGRITY: observational HYPOTHESES for human review, never labels; does not feed the v1.0 G2 gate (circular). Filter to the informative subset with max_window_host_count (e.g. 5) or signal='embedded_quotation_candidate'. v0.73.0.",
    inputSchema: {
      min_confidence: z
        .number()
        .min(0)
        .max(1)
        .optional()
        .describe("identify_composition confidence floor. Default 0.95 (the model's own 'sure' band)."),
      min_shared_chunks: z
        .number()
        .int()
        .min(1)
        .optional()
        .describe("Minimum length-20 windows shared with the composition's exemplar set. Default 1."),
      max_window_host_count: z
        .number()
        .int()
        .min(1)
        .optional()
        .describe("Keep only hits whose RAREST shared window is hosted by ≤ this many tablets — drops the formulaic/boilerplate tail. Try 5 to isolate the informative subset. Default: keep all (classified)."),
      signal: z
        .enum(["embedded_quotation_candidate", "likely_misassignment", "formulaic"])
        .optional()
        .describe("Keep only hits with this signal. 'embedded_quotation_candidate' = the genuinely interesting ones."),
      composition_id: z
        .string()
        .optional()
        .describe("Restrict to one model composition_id (e.g. 'mis_pi')."),
    },
  },
  async ({ min_confidence, min_shared_chunks, max_window_host_count, signal, composition_id }) => {
    const SCHEMA = schemaId("surface_genre_conflicts");
    try {
      const result = surfaceGenreConflicts({
        minConfidence: min_confidence,
        minSharedChunks: min_shared_chunks,
        maxWindowHostCount: max_window_host_count,
        signal,
        compositionId: composition_id,
      });
      const s = result.by_signal;
      const lines: string[] = [
        `surface_genre_conflicts — ${result.stats.corroborated} cross-family hits`,
        ``,
        `  Scanned ${result.stats.assignments_scanned} assignments · ${result.stats.conf_above_threshold} above conf ${result.params.min_confidence} · ${result.stats.cross_family} cross-family · ${result.stats.exemplars_excluded} self-exemplars skipped`,
        ...
        ``,
        `By family-pair:`,
        ...Object.entries(result.by_family_pair)
          .sort((a, b) => b[1] - a[1])
          .map(([k, n]) => `  ${k.padEnd(24)} ${n}`),
        ``,
        `Most informative first (rarest shared window → most discriminating):`,
        ...result.conflicts.slice(0, 15).map(
          (c) =>
            `  [${c.signal}] ${c.tablet_id.padEnd(14)} → ${c.composition_id} (${c.composition_family}-in-${c.ebl_family}) · rarest_window_host=${c.rarest_window_host_count} · ${c.shared_window_count} shared / ${c.tablet_window_count} (frac ${c.overlap_fraction}) · ex ${c.matched_exemplar}`,
        ),
        ``,
        `⚠ Hypotheses for human review, not labels. 'formulaic' = boilerplate (weak). Corroboration is NOT model-independent (identify_composition is chunk-weighted); only window rarity is informative. Does not feed the v1.0 G2 gate.`,
      ];
      return structuredResult(lines.join("\n"), {
        schema: SCHEMA,
        data: result,
        provenance: provenance("local", "local:genre-conflict-sentinel", VERSION, {
          citation:
            "Genre-family disagreements between identify_composition and eBL editorial genre, classified by shared-window rarity into formulaic / likely_misassignment / embedded_quotation_candidate. Observational; corroboration is model-entangled; not a label source. v0.73.0.",
        }),
        warnings: result.warnings.length > 0 ? result.warnings : undefined,
      });
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      return structuredResult(`surface_genre_conflicts error: ${msg}`, {
        schema: SCHEMA,
        data: { conflicts: [], by_family_pair: {}, by_signal: {}, stats: {}, params: {}, warnings: [msg] },
        provenance: provenance("local", "local:genre-conflict-sentinel", VERSION),
      });
    }
  },
...
async function main() {
  ...
  process.stderr.write(`cuneiform-mcp v${VERSION} listening on stdio (116 tools)\n`);
}
